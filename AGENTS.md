...
* Lint / CI gate: there are no unit-test files and no test runner. CI (`.github/workflows/ci.yml`) runs `lake build`, then `lake env lean scripts/check_axioms.lean`, then `lake env lean scripts/check_linter_options.lean`. The axiom script audits the kernel axiom footprint of every `ZeroMath` declaration (same data as `#print axioms`). Only Lean's foundational axioms (`propext`, `Classical.choice`, `Quot.sound`) are allowed — this catches `sorry`/`admit` (`sorryAx`), `native_decide`, and declared `axiom`s. The linter-option script walks parsed syntax of every project `.lean` file and rejects any `set_option` of a `linter.*` option (command, term, or tactic form); comments and string literals do not count.
