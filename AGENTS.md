...
* Lint / CI gate: there are no unit-test files and no test runner. CI (`.github/workflows/verify_lean.yml`) only checks that no `.lean` file contains the words `sorry` or `axiom` — keep proofs complete. The build CI (`.github/workflows/ci.yml`) just runs `lake build`.
