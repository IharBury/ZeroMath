...
theorem fromOrdinal_le_successor_of_tryGetElement_eq_some
  ...
  induction index generalizing x with
  | one =>
    exact Peano.successor_le_successor (Peano.zero_le x.toPeano)
  | successor n ih =>
    simp only [Sequences.Progression.tryGetElement] at h
    cases hm : Sequences.Progression.tryGetElement n (toProgression p) with
    | none =>
    ...
    | some y =>
      ...
      · simp only [hle, ↓reduceIte] at hnext
        ...
        have hy_le : y.toPeano.successor ≤ (y + p.commonDifference).toPeano :=
          Peano.successor_le_of_lt hy_lt
        exact Peano.successor_le_successor (Peano.le_trans hn (heq ▸ hy_le))
      · simp only [hle, ↓reduceIte] at hnext
        nomatch hnext
...
theorem toProgression_finite (p : FiniteArithmeticIncreasing) :
  ...
  | some x =>
    ...
    have hle' : p.limit.toPeano.successor ≤ x.toPeano :=
      Peano.le_of_successor_le_successor hle_idx
...
theorem predecessor_toPeano_eq_of_successor (n : Decimal) (hne : ¬ n ≈ zero)
...
theorem tryGetElement_eq_none_of_length_lt (p : FiniteArithmeticIncreasing)
  ...
  have hlt_peano :
    ...
    have hlt' :
      ...
      hlt
    rwa [hto, fromOrdinal_toPeano_eq_fromOrdinal_peano] at hlt'
...
theorem agreesWithMaskedElementsFrom_eq_fromCurrent_tryGetElement
  ...
  | some first =>
    ...
    · have hlt : getLength p < fromOrdinal index := by
        cases trichotomy_or (getLength p) (fromOrdinal index) with
        | inl hlt => exact hlt
        | inr h =>
          cases h with
...
theorem getLength_agreesWithMaskedElements_of_tryFromMaskedElements
  ...
  refine ⟨hsound.1, ?_⟩
  exact (agreesWithMaskedElementsFrom_eq_true_iff p
    OrdinalNatural.Decimal.one elements).mp hsound.2
