...
def absCardinalPeano (a : Decimal) : CardinalNatural.Peano :=
  toCardinalList a.digits.val CardinalNatural.Peano.zero
...
theorem successorList_toCardinalList (a : Sequences.List Digit)
    (accumulator : CardinalNatural.Peano) :
    match successorList a with
    | ⟨digits, true⟩ =>
        toCardinalList digits accumulator.successor =
          (toCardinalList a accumulator).successor
    | ⟨digits, false⟩ =>
        toCardinalList digits accumulator =
          (toCardinalList a accumulator).successor := by
...
theorem toCardinalList_zero_of_allZero {a : Sequences.List Digit} (h : AllZero a) :
    toCardinalList a CardinalNatural.Peano.zero = CardinalNatural.Peano.zero := by
  induction a with
  | empty => rfl
  | firstElement d ds ih =>
      change toCardinalList ds
...
theorem negate_toPeano (x : Decimal) : (-x).toPeano = -(x.toPeano) := by
  ...
  · next h_all =>
    have habs : absCardinalPeano x = CardinalNatural.Peano.zero := by
      simpa [absCardinalPeano] using toCardinalList_zero_of_allZero h_all
...
theorem toCardinalList_normalizeList (a : Sequences.List Digit)
    (ha : a ≠ Sequences.List.empty) :
    toCardinalList (normalizeList a ha).val CardinalNatural.Peano.zero =
  ...
  | firstElement d ds ih =>
      ...
      · by_cases hds : ds = Sequences.List.empty
        · subst hds
          simp [normalizeList, hd, toCardinalList,
...
theorem toPeano_signed_normalizeList (sign : Option Sign) (a : Sequences.List Digit)
  ...
  cases sign with
  | none =>
      exact toCardinalList_normalizeList a ha ▸ rfl
  | some s =>
      cases s with
      | plus =>
          exact toCardinalList_normalizeList a ha ▸ rfl
...
theorem normalize_toPeano (x : Decimal) : x.normalize.toPeano = x.toPeano := by
  ...
  split
  · next hzero =>
      have hmag := toCardinalList_zero_of_allZero hzero
      rw [toCardinalList_normalizeList] at hmag
      rw [toPeano_zero]
...
theorem toCardinalList_lt_tenPow (l : Sequences.List Digit) :
    toCardinalList l CardinalNatural.Peano.zero <
...
theorem toCardinalList_padAtStart_zeroDigit (l : Sequences.List Digit)
    ...
    toCardinalList (Sequences.List.padAtStart l zeroDigit n)
        CardinalNatural.Peano.zero =
      toCardinalList l CardinalNatural.Peano.zero := by
...
theorem absCardinalPeano_lt_of_lessThanAlignedLists_padded {a b : Decimal}
  ...
  have h_padded := LessThanAlignedLists_toCardinalList_lt
    (Sequences.List.padAtStartToSameLength_sameLength a.digits.val b.digits.val
      zeroDigit) h
  change toCardinalList a.digits.val CardinalNatural.Peano.zero <
    toCardinalList b.digits.val CardinalNatural.Peano.zero
  rw [← toCardinalList_padAtStartToSameLength_fst a.digits.val b.digits.val,
    ← toCardinalList_padAtStartToSameLength_snd a.digits.val b.digits.val]
  exact h_padded
...
theorem toCardinalList_ne_zero_of_not_allZero {a : Sequences.List Digit}
    (h : ¬ AllZero a) :
    toCardinalList a CardinalNatural.Peano.zero ≠ CardinalNatural.Peano.zero := by
  induction a with
  ...
  | firstElement d ds ih =>
      ...
      · have hds : ¬ AllZero ds := by
        ...
        rw [hd]
        exact ih hds
      · exact toCardinalList_ne_zero_of_acc_ne_zero ds hd
...
theorem absCardinalPeano_ne_zero_of_not_allZero {a : Decimal}
  ...
  simpa [absCardinalPeano] using toCardinalList_ne_zero_of_not_allZero h
...
theorem toCardinalList_inj_sameLength {l1 l2 : Sequences.List Digit}
  ...
  | firstElement h_tail ih =>
      ...
      rw [h_len] at heq
      have hv1_lt :
          toCardinalList ds1 CardinalNatural.Peano.zero <
            CardinalNatural.Peano.tenPow ds1.length :=
        toCardinalList_lt_tenPow ds1
      have hv2_lt :
          toCardinalList ds2 CardinalNatural.Peano.zero <
        ...
        | inl hlt =>
            ...
            have hchain :
                d1.val * CardinalNatural.Peano.tenPow ds1.length +
                    CardinalNatural.Peano.tenPow ds1.length ≤
                  d1.val * CardinalNatural.Peano.tenPow ds1.length +
                    toCardinalList ds1 CardinalNatural.Peano.zero := by
              have hstep1 :
                  d1.val * CardinalNatural.Peano.tenPow ds1.length +
                      CardinalNatural.Peano.tenPow ds1.length =
              ...
              | firstElement digit rest =>
                  ...
                  | empty =>
                      have hdig : digit.val = CardinalNatural.Peano.zero := by
                        simp only [absCardinalPeano, toCardinalList,
...
theorem leadingDigit_ne_zero_of_normalized_ne_zero_abs
  ...
  | none =>
      ...
      | empty =>
          intro hdig
          apply hne_abs
          simp only [absCardinalPeano, toCardinalList, hdig,
            CardinalNatural.Peano.zero_multiply, CardinalNatural.Peano.zero_add]
      | firstElement _ _ =>
          simp only [isNormalized] at hnorm
                  ...
                  | firstElement da das =>
                      ...
                      | firstElement db dbs =>
                          ...
                          rw [toCardinalList_firstElement,
                            toCardinalList_firstElement] at heq_split
                                ...
                                have hval_lt :
                                  ...
                                  have hstep1 :
                                    ...
                                    CardinalNatural.Peano.add_lt_add_left
                                      (toCardinalList_lt_tenPow das) _
                                  have hstep2 :
                                      da.val * CardinalNatural.Peano.tenPow das.length +
                                          CardinalNatural.Peano.tenPow das.length =
                                        da.val.successor *
                                      ...
                                      have hstep1 :
                                          db.val * CardinalNatural.Peano.tenPow dbs.length +
                                            ...
                                            db.val * CardinalNatural.Peano.tenPow dbs.length +
                                              CardinalNatural.Peano.tenPow dbs.length :=
                                        CardinalNatural.Peano.add_lt_add_left
                                          (toCardinalList_lt_tenPow dbs) _
                                      have hstep2 :
                                          db.val * CardinalNatural.Peano.tenPow dbs.length +
                                              CardinalNatural.Peano.tenPow dbs.length =
                                            db.val.successor *
                          ...
                          exact toCardinalList_inj_sameLength hsl heq_raw
...
theorem toCardinalList_of_successorList (a : Sequences.List Digit) :
    match successorList a with
    | ⟨digits, true⟩ =>
        toCardinalList digits CardinalNatural.Peano.one =
          (toCardinalList a CardinalNatural.Peano.zero).successor
    | ⟨digits, false⟩ =>
        toCardinalList digits CardinalNatural.Peano.zero =
          (toCardinalList a CardinalNatural.Peano.zero).successor := by
  have h := successorList_toCardinalList a CardinalNatural.Peano.zero
...
theorem successor_toPeano_none (x : Decimal) (hsign : x.sign = none) :
  ...
  · next sign _ =>
      split
      · next digits hsucc =>
          have h_list := toCardinalList_of_successorList x.digits.val
          rw [hsucc] at h_list
          ...
          have habs :
            ...
            simpa [absCardinalPeano, toCardinalList,
...
theorem successor_toPeano_minus (x : Decimal) (hsign : x.sign = some Sign.minus) :
  ...
  · next h_eq =>
      ...
      · next digits hpred =>
          ...
          have h_abs :
              absCardinalPeano x =
                (toCardinalList digits CardinalNatural.Peano.zero).successor := by
            have hsucc :=
              successorList_toCardinalList digits CardinalNatural.Peano.zero
            rw [h_succ_pred] at hsucc
            dsimp only at hsucc
            simpa [absCardinalPeano] using hsucc
          split
          · next h_zero =>
              have hdigits0 := toCardinalList_zero_of_allZero h_zero
          ...
          · next h_zero =>
              ...
              exact
                (congrArg Peano.successor
                  (Peano.neg_succ
                    (Peano.fromCardinalNatural
                      (toCardinalList digits CardinalNatural.Peano.zero)))).trans
                  (Peano.succ_pred _)
  · next sign h_ne =>
...
theorem not_allZero_normalizeList_of_not_allZero {a : Sequences.List Digit}
  ...
  have hmag := toCardinalList_zero_of_allZero hzero
  rw [toCardinalList_normalizeList] at hmag
  exact toCardinalList_ne_zero_of_not_allZero h hmag
              ...
              · next _ =>
                  ...
                  · next digs hsucc =>
                      ...
                      have ha0 : ¬ AllZero a.digits.val := by
                        intro hall
                        have hmag := toCardinalList_of_successorList digits
                        rw [h_succ] at hmag
                        dsimp only at hmag
                        have hzero := toCardinalList_zero_of_allZero hall
                        rw [hzero] at hmag
...
theorem addMagnitudes_toPeano (sign : Option Sign) (a b : Sequences.List Digit) :
  ...
  · next digits h_add =>
      ...
      have h_list :
          toCardinalList
            ...
            toCardinalList a CardinalNatural.Peano.zero +
              toCardinalList b CardinalNatural.Peano.zero := by
        rw [toCardinalList_firstElement, hone, h_length,
          CardinalNatural.Peano.one_multiply, CardinalNatural.Peano.add_commutative,
          h_value, toCardinalList_padAtStartToSameLength_fst,
  ...
  · next digits h_add =>
      ...
      · next heq =>
          subst heq
          have h_list :
              toCardinalList Sequences.List.empty CardinalNatural.Peano.zero =
                toCardinalList a CardinalNatural.Peano.zero +
                  toCardinalList b CardinalNatural.Peano.zero := by
          ...
          cases sign with
          | none =>
              simp [toPeano_zero, toCardinalList] at h_list ⊢
  ...
  | mk digits borrow =>
      ...
      | false =>
          ...
          · next heq =>
              ...
              have h_spec' := h_value
              simp [toCardinalList] at h_spec'
              have h_sum : absCardinalPeano smaller = absCardinalPeano larger := by
                unfold absCardinalPeano
                rw [← toCardinalList_padAtStartToSameLength_fst larger.digits.val
                  smaller.digits.val,
                  ← toCardinalList_padAtStartToSameLength_snd larger.digits.val
                    smaller.digits.val]
...
theorem toCardinalList_padAtEnd (l : Sequences.List Digit) (n : CardinalNatural.Peano) :
  ...
  | firstElement d ds ih =>
    simp only [Sequences.List.padAtEnd, toCardinalList_firstElement,
...
theorem toCardinalList_addAlignedLists_result {a b : Sequences.List Digit}
  ...
  | mk digits carry =>
      ...
      | true =>
          ...
          rw [toCardinalList_firstElement, oneDigit, CardinalNatural.Peano.one_multiply,
            h_length]
          rw [CardinalNatural.Peano.add_commutative]
          exact h_value
          ...
          | mk digits carry =>
              ...
              · have h_add_value := toCardinalList_addAlignedLists_result h_same
                ...
                | false =>
                    ...
                    rw [toCardinalList_padAtEnd, toCardinalList_multiplyListByDigit,
                      ih_value, h_shift]
                    rw [toCardinalList_firstElement]
                    calc
                      toCardinalList a CardinalNatural.Peano.zero *
...
theorem multiply_toPeano (x y : Decimal) :
  ...
  · next hd =>
      split
      · next hzero =>
          have hmag0 := toCardinalList_zero_of_allZero hzero
          rw [toCardinalList_normalizeList] at hmag0
          ...
          | false =>
              ...
              | false =>
                  ...
                  change _ = Peano.fromCardinalNatural
                    (toCardinalList x.digits.val CardinalNatural.Peano.zero *
                      toCardinalList y.digits.val CardinalNatural.Peano.zero)
                  rw [hprod]
                  rfl
              | true =>
                  have ⟨hy_peano, _⟩ := toPeano_eq_negate_fromCardinal_of_isNegative y hy
