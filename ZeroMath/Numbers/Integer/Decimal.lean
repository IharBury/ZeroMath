...
/-- Attach a sign to a non-empty digit list, canonicalizing `some Sign.plus` to `none`
and any zero magnitude (including `-0`) to `zero`. -/
def withSign (sign : Option Sign) (digits : NonEmptyList) : Decimal :=
  if AllZero digits.val then
    zero
  else
    match sign with
    | some Sign.plus | none => ⟨none, digits⟩
    | some Sign.minus => ⟨some Sign.minus, digits⟩

/-- Strip leading zeros via `Digits.normalizeList`, then attach a canonical sign.
Empty input yields `zero`. -/
def ofSignedList (sign : Option Sign) (a : Sequences.List Digit) : Decimal :=
  if ha : a = Sequences.List.empty then
    zero
  else
    withSign sign (normalizeList a ha)

def normalize (a : Decimal) : Decimal :=
  withSign a.sign (normalizeList a.digits.val a.digits.property)
...
theorem withSign_plus_eq_none (digits : NonEmptyList) :
    withSign (some Sign.plus) digits = withSign none digits := by
  unfold withSign
  split <;> rfl

theorem ofSignedList_plus_eq_none (a : Sequences.List Digit) :
    ofSignedList (some Sign.plus) a = ofSignedList none a := by
  unfold ofSignedList
  split
  · rfl
  · exact withSign_plus_eq_none _
...
theorem withSign_toPeano (sign : Option Sign) (digits : NonEmptyList) :
    toPeano (withSign sign digits) =
      match sign with
      | some Sign.minus =>
          Peano.negate (Peano.fromCardinalNatural
            (toCardinalPeanoList digits.val CardinalNatural.Peano.zero))
      | _ =>
          Peano.fromCardinalNatural
            (toCardinalPeanoList digits.val CardinalNatural.Peano.zero) := by
  unfold withSign
  by_cases hzero : AllZero digits.val
  · simp only [hzero, ↓reduceIte]
    have hmag := toCardinalPeanoList_zero_of_allZero hzero
    rw [toPeano_zero, hmag]
    cases sign with
    | none => rfl
    | some s =>
        cases s with
...
theorem ofSignedList_toPeano (sign : Option Sign) (a : Sequences.List Digit) :
    toPeano (ofSignedList sign a) =
      match sign with
      | some Sign.minus =>
          Peano.negate (Peano.fromCardinalNatural
  ...
  · next heq =>
      ...
      | some s =>
          ...
          | minus => rfl
  · next ha =>
      rw [withSign_toPeano, toCardinalPeanoList_normalizeList]

theorem normalize_toPeano (x : Decimal) : x.normalize.toPeano = x.toPeano := by
  have hx : x.normalize = ofSignedList x.sign x.digits.val := by
    unfold normalize ofSignedList
    simp [x.digits.property]
  rw [hx, ofSignedList_toPeano]
...
theorem withSign_isNormalized (sign : Option Sign) (digits : NonEmptyList)
    (hnorm : isNormalized ⟨none, digits⟩ = true ∨ AllZero digits.val) :
    (withSign sign digits).isNormalized = true := by
  unfold withSign
  by_cases hzero : AllZero digits.val
  · simp only [hzero, ↓reduceIte]
    rfl
  · simp only [hzero, ↓reduceIte]
    have h : isNormalized ⟨none, digits⟩ = true :=
      match hnorm with
      | Or.inl h => h
      | Or.inr h => False.elim (hzero h)
    cases sign with
    | none => exact h
    | some s =>
        cases s with
        | plus => exact h
        | minus =>
            -- Same digit check as the non-negative form once the leading digit is nonzero.
            match digits with
...
theorem ofSignedList_isNormalized (sign : Option Sign) (a : Sequences.List Digit) :
    (ofSignedList sign a).isNormalized = true := by
  unfold ofSignedList
  split
  · rfl
  · next ha =>
      exact withSign_isNormalized sign (normalizeList a ha)
        (Or.inl (normalizeList_isNormalized_digits a ha))

theorem normalize_isNormalized (d : Decimal) : d.normalize.isNormalized = true := by
  unfold normalize
  exact withSign_isNormalized d.sign (normalizeList d.digits.val d.digits.property)
    (Or.inl (normalizeList_isNormalized_digits d.digits.val d.digits.property))
...
theorem withSign_eq_zero_of_allZero (sign : Option Sign) {digits : NonEmptyList}
    (h : AllZero digits.val) : withSign sign digits = zero := by
  simp [withSign, h]

theorem ofSignedList_eq_zero_of_allZero (sign : Option Sign) {a : Sequences.List Digit}
    (h : AllZero a) : ofSignedList sign a = zero := by
  unfold ofSignedList
  split
  · rfl
  · next ha =>
      rw [withSign_eq_zero_of_allZero sign]
      have hnorm := Digits.normalizeList_eq_zero_of_allZero ha h
      simp [AllZero, zeroDigit, hnorm]

theorem normalize_eq_zero_of_allZero (a : Decimal) (h : AllZero a.digits.val) :
    a.normalize = zero := by
  unfold normalize
  rw [withSign_eq_zero_of_allZero a.sign]
...
theorem withSign_normalizeList_cons_zero (sign : Option Sign) (d : Digit)
    (ds : Sequences.List Digit) (hd : d.val = CardinalNatural.Peano.zero)
    (hds : ds ≠ Sequences.List.empty) :
    withSign sign (normalizeList (Sequences.List.firstElement d ds) (by simp)) =
      withSign sign (normalizeList ds hds) := by
  rw [normalizeList_cons_zero d ds hd hds]

theorem ofSignedList_cons_zero (sign : Option Sign) (d : Digit) (ds : Sequences.List Digit)
    (hd : d.val = CardinalNatural.Peano.zero) :
    ofSignedList sign (Sequences.List.firstElement d ds) = ofSignedList sign ds := by
  cases ds with
  | empty =>
      unfold ofSignedList
      have hne : Sequences.List.firstElement d Sequences.List.empty ≠ Sequences.List.empty := by
        simp
      simp only [hne, ↓reduceDIte]
      have hnorm :
          normalizeList (Sequences.List.firstElement d Sequences.List.empty) hne =
            ⟨Sequences.List.firstElement d Sequences.List.empty, hne⟩ := by
        simp [normalizeList, hd]
      have hall : AllZero (Sequences.List.firstElement d Sequences.List.empty) := by
        simp [AllZero, hd]
      rw [hnorm, withSign_eq_zero_of_allZero sign hall]
  | firstElement d' ds' =>
      unfold ofSignedList
      have hne : Sequences.List.firstElement d (Sequences.List.firstElement d' ds') ≠
          Sequences.List.empty := by simp
      have hds : Sequences.List.firstElement d' ds' ≠ Sequences.List.empty := by simp
      simp only [hne, hds, ↓reduceDIte]
      exact withSign_normalizeList_cons_zero sign d (Sequences.List.firstElement d' ds') hd hds
  ...
  · cases hsign : x.sign with
    ...
    | some s =>
        ...
        | plus =>
            ...
            unfold normalize
            rw [hsign, withSign_plus_eq_none]
        | minus =>
...
theorem ofSignedList_minus_of_successorList_allZero
    {digits a : Sequences.List Digit}
    (h : successorList digits = ⟨a, false⟩) (hzero : AllZero digits) :
    ofSignedList (some Sign.minus) a = minusOne := by
  ...
  | firstElement d ds ih =>
      ...
      | mk digs carry =>
          ...
          | false =>
              ...
              rw [ofSignedList_cons_zero (some Sign.minus) d digs hd]
      ...
      · next digits hsucc =>
          ...
          · next sign' _ =>
              ...
              · next digs hpred =>
                  ...
                  unfold normalize
                  rw [hsign, withSign_normalizeList_cons_zero none zeroDigit a.digits.val rfl
                    a.digits.property]
      · next digits hsucc =>
      ...
      · next digits hsucc =>
          ...
          · next sign' _ =>
              ...
              · next digs hpred =>
                  ...
                  simp only [hnz, ↓reduceIte]
                  unfold normalize
                  rw [hsign, withSign_normalizeList_cons_zero (some Sign.plus) zeroDigit
                    a.digits.val rfl a.digits.property]
      · next digits hsucc =>
          have h_pred : predecessorList digits = ⟨a.digits.val, false⟩ := by
            have h' := predecessorList_successorList a.digits.val
            simpa [hsucc] using h'
      ...
      · next digits hpred =>
          ...
          · -- AllZero digits → successor = zero → predecessor = minusOne
            next hzero =>
              ...
              unfold normalize
              rw [hsign]
              have h_ne : a.digits.val ≠ Sequences.List.empty := a.digits.property
              have hnorm := ofSignedList_minus_of_successorList_allZero h_succ hzero
              -- ofSignedList minus a.digits = withSign minus (normalizeList a.digits)
              simp [ofSignedList, h_ne] at hnorm
              exact hnorm.symm
...
theorem ofSignedList_of_successorList_allZero
    (sign : Option Sign) {digits a : Sequences.List Digit}
    (h : successorList digits = ⟨a, false⟩) (hzero : AllZero digits) :
    ofSignedList sign a =
      ofSignedList sign (Sequences.List.firstElement oneDigit Sequences.List.empty) := by
  induction digits generalizing a with
  | empty =>
      cases h
  | firstElement d ds ih =>
      ...
      | mk digs carry =>
          ...
          | false =>
              simp [hds] at h
              cases h
              rw [ofSignedList_cons_zero sign d digs hd]
              exact ih hds hzero.2
  ...
  · next sign _ =>
      ...
      · next digits hpred =>
          ...
          · -- AllZero digits: predecessor = zero; successor zero = one
            next hzero =>
              rw [successor_zero]
              -- one.normalize = one; a.normalize = withSign none (normalizeList a.digits) = one
              unfold normalize one
              rw [hsign]
              have hnorm := ofSignedList_of_successorList_allZero none h_succ hzero
              have h_ne : a.digits.val ≠ Sequences.List.empty := a.digits.property
              simp [ofSignedList, h_ne, oneDigit] at hnorm ⊢
              exact hnorm.symm
  ...
  · next sign _ =>
      ...
      · next digits hpred =>
          ...
          · next hzero =>
              ...
              rw [hsign]
              have hnorm :=
                ofSignedList_of_successorList_allZero (some Sign.plus) h_succ hzero
              have h_ne : a.digits.val ≠ Sequences.List.empty := a.digits.property
              -- ofSignedList plus [1] = ⟨none, [1]⟩ = one form
              simp [ofSignedList, h_ne, oneDigit, withSign_plus_eq_none] at hnorm ⊢
              exact hnorm.symm
          · next hzero =>
              unfold successor
              split
        ...
        next digits hsucc =>
          ...
          · next _ =>
              ...
              · next digs hpred =>
                  ...
                  simp only [hnz, ↓reduceIte]
                  unfold normalize
                  rw [hsign, withSign_normalizeList_cons_zero (some Sign.minus) zeroDigit
                    a.digits.val rfl a.digits.property]
          · next sign hne =>
              exact False.elim (hne rfl)
...
/-- Add two digit lists as magnitudes and attach the given sign (via `ofSignedList`). -/
def addMagnitudes (sign : Option Sign) (a b : Sequences.List Digit) : Decimal :=
  ...
  | ⟨digits, false⟩ =>
      ofSignedList sign digits

/-- Subtract digit magnitudes `|larger| - |smaller|` when `|smaller| < |larger|`,
    attaching the given sign via `ofSignedList`. -/
...
def multiply (a b : Decimal) : Decimal :=
  let sign : Option Sign :=
    ...
    | _, _ => none
  ofSignedList sign (multiplyList a.digits.val b.digits.val).1
...
theorem addMagnitudes_toPeano (sign : Option Sign) (a b : Sequences.List Digit) :
  ...
  · next digits h_add =>
      ...
      simp at h_value
      have h_norm := ofSignedList_toPeano sign
        (Sequences.List.firstElement oneDigit digits)
      rw [h_norm]
      have h_list :
          toCardinalPeanoList
  ...
  · next digits h_add =>
      ...
      simp at h_value
      have h_norm := ofSignedList_toPeano sign digits
      rw [h_norm]
      have h_list :
          toCardinalPeanoList digits CardinalNatural.Peano.zero =
            toCardinalPeanoList a CardinalNatural.Peano.zero +
...
theorem addPartialListDigit_spec (a : Sequences.List Digit) (b : Digit) :
    ...
    toCardinalPeanoList (addPartialListDigit a b).1 CardinalNatural.Peano.zero +
        (addPartialListDigit a b).2.val * CardinalNatural.Peano.tenPow a.length =
      toCardinalPeanoList a CardinalNatural.Peano.zero + b.val := by
  induction a with
  | empty =>
      simp [addPartialListDigit, toCardinalPeanoList, Sequences.List.length,
  ...
  | firstElement d ds ih =>
      ...
      | mk digits carry =>
          ...
          · constructor
            · simp [Sequences.List.length, h_length]
            · simp only [toCardinalPeanoList_firstElement, Sequences.List.length, zeroDigit,
                CardinalNatural.Peano.zero_multiply, CardinalNatural.Peano.add_zero]
              rw [h_length]
              calc
                ...
                _ = d.val * CardinalNatural.Peano.tenPow ds.length +
                      ...
                      rw [CardinalNatural.Peano.add_commutative
                        ...
                        (toCardinalPeanoList digits CardinalNatural.Peano.zero)]
                _ = d.val * CardinalNatural.Peano.tenPow ds.length +
                      (toCardinalPeanoList ds CardinalNatural.Peano.zero + b.val) := by
                      rw [ih_value]
                _ = d.val * CardinalNatural.Peano.tenPow ds.length +
                      toCardinalPeanoList ds CardinalNatural.Peano.zero + b.val := by
                      rw [CardinalNatural.Peano.add_associative]
          · next h_not_lt =>
            constructor
            · simp [Sequences.List.length, h_length]
            · simp only [toCardinalPeanoList_firstElement, Sequences.List.length, oneDigit,
...
theorem toCardinalPeanoList_padAtEnd (l : Sequences.List Digit) (n : CardinalNatural.Peano) :
    toCardinalPeanoList (Sequences.List.padAtEnd l zeroDigit n) CardinalNatural.Peano.zero =
      toCardinalPeanoList l CardinalNatural.Peano.zero * CardinalNatural.Peano.tenPow n := by
  induction l with
  | empty =>
    induction n with
    | zero =>
      simp [Sequences.List.padAtEnd, toCardinalPeanoList, CardinalNatural.Peano.tenPow,
        CardinalNatural.Peano.multiply_one]
    | successor n ih =>
      simp [Sequences.List.padAtEnd, toCardinalPeanoList_firstElement, toCardinalPeanoList]
      show CardinalNatural.Peano.zero * _ + _ = _
      simp only [CardinalNatural.Peano.zero_multiply, CardinalNatural.Peano.zero_add]
      rw [ih]
      simp [toCardinalPeanoList, CardinalNatural.Peano.zero_multiply]
  | firstElement d ds ih =>
    simp only [Sequences.List.padAtEnd, toCardinalPeanoList_firstElement,
      Sequences.List.padAtEnd_length]
    rw [CardinalNatural.Peano.tenPow_add, ← CardinalNatural.Peano.multiply_associative, ih,
        ← CardinalNatural.Peano.multiply_distributive_over_add_left,
        ← toCardinalPeanoList_firstElement]

theorem toCardinalPeanoList_addListDigit (a : Sequences.List Digit) (b : Digit) :
    toCardinalPeanoList (addListDigit a b) CardinalNatural.Peano.zero =
      toCardinalPeanoList a CardinalNatural.Peano.zero + b.val := by
  ...
  | mk ds carry =>
    ...
    · rw [h_carry, CardinalNatural.Peano.zero_multiply, CardinalNatural.Peano.add_zero] at h_val
      rw [if_pos h_carry]
      exact h_val
    · rw [if_neg h_carry, toCardinalPeanoList_firstElement, h_len,
          CardinalNatural.Peano.add_commutative (carry.val * _)]
      exact h_val

theorem toCardinalPeanoList_multiplyDigitsPeano (d : Digit) (n : CardinalNatural.Peano) :
    toCardinalPeanoList (multiplyDigitsPeano d n) CardinalNatural.Peano.zero =
      d.val * n := by
  induction n with
  | zero =>
    simp [multiplyDigitsPeano, toCardinalPeanoList_firstElement, toCardinalPeanoList, zeroDigit,
          Sequences.List.length, CardinalNatural.Peano.tenPow,
          CardinalNatural.Peano.multiply_zero, CardinalNatural.Peano.multiply_one]
  | successor n ih =>
    unfold multiplyDigitsPeano
    rw [toCardinalPeanoList_addListDigit, ih, CardinalNatural.Peano.multiply_successor]

theorem toCardinalPeanoList_multiplyDigits (d b : Digit) :
    toCardinalPeanoList (multiplyDigits d b) CardinalNatural.Peano.zero = d.val * b.val := by
  unfold multiplyDigits
  exact toCardinalPeanoList_multiplyDigitsPeano d b.val

theorem toCardinalPeanoList_addListDigit_multiplyDigits (d b carry : Digit) :
    toCardinalPeanoList (addListDigit (multiplyDigits d b) carry) CardinalNatural.Peano.zero =
      d.val * b.val + carry.val := by
  rw [toCardinalPeanoList_addListDigit, toCardinalPeanoList_multiplyDigits]

theorem multiplyPartialListByDigit_spec (a : Sequences.List Digit) (d : Digit) :
    (multiplyPartialListByDigit a d).1.length = a.length ∧
    toCardinalPeanoList (multiplyPartialListByDigit a d).1 CardinalNatural.Peano.zero +
        (multiplyPartialListByDigit a d).2.val * CardinalNatural.Peano.tenPow a.length =
      toCardinalPeanoList a CardinalNatural.Peano.zero * d.val := by
  induction a with
  | empty =>
      simp [multiplyPartialListByDigit, toCardinalPeanoList, Sequences.List.length,
  ...
  | firstElement digit ds ih =>
      ...
      | mk digits carry =>
          ...
          dsimp only at ih
          obtain ⟨h_length, ih_value⟩ := ih
          have h_withCarry_value := toCardinalPeanoList_addListDigit_multiplyDigits digit d carry
          split
          · next h_withCarry =>
                ...
                calc
                  ...
                  _ = digit.val * d.val * CardinalNatural.Peano.tenPow ds.length +
                      ...
                      rw [CardinalNatural.Peano.add_commutative
                        ...
                        (toCardinalPeanoList digits CardinalNatural.Peano.zero)]
                  _ = digit.val * d.val * CardinalNatural.Peano.tenPow ds.length +
                        (toCardinalPeanoList ds CardinalNatural.Peano.zero * d.val) := by
                      rw [ih_value]
                  _ = (digit.val * CardinalNatural.Peano.tenPow ds.length +
                        toCardinalPeanoList ds CardinalNatural.Peano.zero) * d.val := by
                      rw [CardinalNatural.Peano.multiply_distributive_over_add_left]
                      rw [CardinalNatural.Peano.multiply_associative]
                      rw [CardinalNatural.Peano.multiply_commutative d.val
                        (CardinalNatural.Peano.tenPow ds.length)]
                      rw [← CardinalNatural.Peano.multiply_associative digit.val
                        (CardinalNatural.Peano.tenPow ds.length) d.val]
              ...
              · rw [h_withCarry] at h_withCarry_value
                ...
                calc
                  ...
                  _ = (x.val * CardinalNatural.Peano.ten + y.val) *
                        CardinalNatural.Peano.tenPow ds.length +
                        toCardinalPeanoList digits CardinalNatural.Peano.zero := by
                      rw [CardinalNatural.Peano.add_commutative y.val
                        (x.val * CardinalNatural.Peano.ten)]
          ...
          · next x y z zs h_withCarry =>
              exact False.elim
                (addListDigit_multiplyDigits_not_three_or_more digit d carry x y z zs h_withCarry)

theorem toCardinalPeanoList_multiplyListByDigit (a : Sequences.List Digit) (d : Digit) :
    toCardinalPeanoList (multiplyListByDigit a d) CardinalNatural.Peano.zero =
      toCardinalPeanoList a CardinalNatural.Peano.zero * d.val := by
...
theorem toCardinalPeanoList_addAlignedLists_result {a b : Sequences.List Digit}
  (h : Sequences.List.SameLength a b) :
  let result := addAlignedLists a b h
  let digitsWithCarry :=
    if result.2 then Sequences.List.firstElement oneDigit result.1 else result.1
  toCardinalPeanoList digitsWithCarry CardinalNatural.Peano.zero =
    toCardinalPeanoList a CardinalNatural.Peano.zero +
      toCardinalPeanoList b CardinalNatural.Peano.zero := by
  ...
  | mk digits carry =>
      ...
      | true =>
          ...
          rw [toCardinalPeanoList_firstElement, oneDigit, CardinalNatural.Peano.one_multiply,
            h_length]
          rw [CardinalNatural.Peano.add_commutative]
          exact h_value
...
theorem multiplyList_spec (a b : Sequences.List Digit) :
    ...
    toCardinalPeanoList (multiplyList a b).1 CardinalNatural.Peano.zero =
      toCardinalPeanoList a CardinalNatural.Peano.zero *
        toCardinalPeanoList b CardinalNatural.Peano.zero := by
  induction b with
  | empty =>
      simp [multiplyList, toCardinalPeanoList, Sequences.List.length,
  ...
  | firstElement d ds ih =>
      ...
      | mk accumulator shift =>
          ...
          | mk digits carry =>
              ...
              · have h_add_value := toCardinalPeanoList_addAlignedLists_result h_same
                ...
                | false =>
                    ...
                    rw [h_add_value]
                    rw [toCardinalPeanoList_padAtStartToSameLength_fst,
                        toCardinalPeanoList_padAtStartToSameLength_snd]
                    dsimp only [withShift, digitProduct]
                    rw [toCardinalPeanoList_padAtEnd, toCardinalPeanoList_multiplyListByDigit,
                      ih_value, h_shift]
                    rw [toCardinalPeanoList_firstElement]
                    calc
                      toCardinalPeanoList a CardinalNatural.Peano.zero *
                            toCardinalPeanoList ds CardinalNatural.Peano.zero +
                          toCardinalPeanoList a CardinalNatural.Peano.zero * d.val *
                            CardinalNatural.Peano.tenPow ds.length =
                        toCardinalPeanoList a CardinalNatural.Peano.zero *
                          (toCardinalPeanoList ds CardinalNatural.Peano.zero +
                          ...
                          rw [CardinalNatural.Peano.multiply_associative]
                      _ = toCardinalPeanoList a CardinalNatural.Peano.zero *
                          (d.val * CardinalNatural.Peano.tenPow ds.length +
                            toCardinalPeanoList ds CardinalNatural.Peano.zero) := by
                          rw [CardinalNatural.Peano.add_commutative]
                | true =>
