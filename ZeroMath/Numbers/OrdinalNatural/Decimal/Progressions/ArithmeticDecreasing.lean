...
theorem lt_of_next_eq_some (p : ArithmeticDecreasing) (y x : Decimal)
  ...
  | some z =>
    ...
    · simp only [hle, ↓reduceIte] at h
      ...
      exact heq ▸ Peano.lt_of_successor_le hz_le
    · simp only [hle, ↓reduceIte] at h
      nomatch h
...
theorem toProgression_finite (p : ArithmeticDecreasing) :
  ...
  | some first =>
    ...
    | some x =>
      have hle :=
        add_le_successor_first_of_tryGetElement p first
...
theorem predecessor_multiply_eq_add_of_not_one (index diff : Decimal)
  ...
  have hsucc :=
    toPeano_eq_successor_predecessor_toPeano
      (index.predecessor hne) hpred
  rw [hsucc, Peano.successor_multiply]
...
theorem trySubtract_multiply_of_tryGetElement_eq_some
    ...
    Option.Rel (· ≈ ·)
      ...
      (some x) := by
  have hpeano :=
    toPeano_eq_successor_predecessor_toPeano index hne
    ...
    | some z =>
      ...
      · simp only [hle_lim, ↓reduceIte] at hnext
        ...
        if hpred : index.predecessor hne ≈ one then
          ...
          have ih :=
            trySubtract_multiply_of_tryGetElement_eq_some p start hf
...
theorem eq_getElementFrom_of_tryGetElement_eq_some
  ...
  if hone : index ≈ one then
    ...
    have hrel :=
      trySubtract_multiply_of_tryGetElement_eq_some p start hf index x hone h
      ...
      · rw [show (if d ≈ diff then
        ...
        · have h1 :
          ...
          have h2 :
            ...
            exact
              (Peano.Progressions.ArithmeticDecreasing.lastElementFrom_successor_successor_of_trySubtract
        ...
        · intro next hs_next
          ...
          | some next' =>
            ...
            obtain ⟨hne, hexpand, hpred⟩ :=
              getElementsFrom_successor_of_trySubtract x diff next'
                (CardinalNatural.Decimal.fromPeano
                  (Sequences.List.firstElement x xs).length)
                xs.length hn hsx
            have hpred_eq :
    ...
    | some diff =>
      ...
      | some last =>
        ...
        obtain ⟨hne_n, hexpand, hpred⟩ :=
          getElementsFrom_successor_of_trySubtract x diff y' n
...
theorem trySubtract_rel_commutative (diff first next : Decimal)
  ...
  have hgen :
    ...
    | successor k ih =>
      ...
      have hexpand :
        ...
        conv => lhs; unfold getElementsFrom
        simp only [hne, ↓reduceDIte]
      have hrel := trySubtract_rel_commutative subtractiveCommonDifference prev next hs
        ...
        have hlast :
            Peano.Progressions.ArithmeticDecreasing.lastElementFrom
                ...
                (CardinalNatural.Peano.successor
                  (CardinalNatural.Peano.successor k)) =
              next.toPeano := by
          rw [Peano.Progressions.ArithmeticDecreasing.lastElementFrom_successor_successor_of_trySubtract
...
theorem tryFromElements_getElementsFrom_ge_two
  ...
  obtain ⟨m, hm⟩ := CardinalNatural.Peano.eq_successor_successor_of_two_le n.toPeano hge
  obtain ⟨next, hs⟩ :=
    trySubtract_eq_some_of_getElementsFrom_length_ge_two first
      subtractiveCommonDifference n hLen
  obtain ⟨hne, hexpand, hpred⟩ :=
    getElementsFrom_successor_of_trySubtract first subtractiveCommonDifference next n
  ...
  have hrel := trySubtract_rel_commutative subtractiveCommonDifference first next hs
...
theorem getElementFrom_eq_add_multiply_of_lt (p : ArithmeticDecreasing)
  ...
  have hle_peano :
    ...
    exact hle''
  exact
    Peano.Progressions.ArithmeticDecreasing.getElementFrom_eq_add_multiply_of_lt
...
theorem tryCommonDifferenceFromOrderedIndexedElements_getElementFrom
  ...
  obtain ⟨d, hdiv_eq, hdiv_approx⟩ :=
    InfiniteArithmetic.exists_of_option_rel_some
      (InfiniteArithmetic.tryDivide_of_equivalent_multiply hsub_approx)
    ...
    · have hfirst_prog : (toProgression p).first = some first := by
      ...
      have htry :=
        Sequences.Progression.tryGetElement_eq_some_getElement
          (toProgression p) (toProgression_finite p) index.toPeano hle_prog
      have hrel :=
        trySubtract_multiply_of_tryGetElement_eq_some p first hfirst_prog index
          (Sequences.Progression.getElement (toProgression p)
...
theorem getElementFrom_of_tryFirstFromIndexedElement
    (index element subtractiveCommonDifference first : Decimal)
  ...
  if hone : index ≈ one then
    ...
    injection h with heq
    have hrel :=
      InfiniteArithmetic.trySubtract_add_right_of_equivalent element
        ((index.predecessor hone) * subtractiveCommonDifference)
        ((index.predecessor hone) * subtractiveCommonDifference) (Setoid.refl _)
...
theorem eq_subtract_multiply_of_tryCommonDifferenceFromOrderedIndexedElements
  ...
  | some elementDiff =>
    ...
    have hmul : elementDiff ≈ (subtract index' index hlt) * diff :=
      equivalent_of_tryDivide_multiply elementDiff (subtract index' index hlt) diff h
...
theorem getElementFrom_of_tryFirst_tryCommonDifference
    (index element index' element' : Decimal) (hlt : index < index')
    (diff first : Decimal) (length : CardinalNatural.Decimal)
  ...
  have hf :
    ...
    effectiveFirst_of_equivalent_lastElementFrom first diff diff
      (lastElementFrom first diff length) length hne (Setoid.refl _)
  have hlenp := getLength_lastElementFrom first diff length hne hlen
  have hle2p :
      CardinalNatural.Decimal.fromOrdinal index' ≤ getLength p :=
    CardinalNatural.Decimal.le_of_le_of_equivalent hle' (Setoid.symm hlenp)
  have hstep :=
    getElementFrom_eq_add_multiply_of_lt p first hf index index' hlt hle2p
  -- getElementFrom index ≈ getElementFrom index' + gap * diff
  -- getElementFrom index ≈ element
  -- element ≈ element' + gap * diff
  apply equivalent_of_toPeano_eq
  have hleft :=
  ...
  have hcancel :
      (getElementFrom first diff index').toPeano +
          ((subtract index' index hlt) * diff).toPeano =
        element'.toPeano + ((subtract index' index hlt) * diff).toPeano := by
    calc
      (getElementFrom first diff index').toPeano +
          ((subtract index' index hlt) * diff).toPeano
          = (getElementFrom first diff index' +
              (subtract index' index hlt) * diff).toPeano := by
            rw [add_toPeano]
      _ = element.toPeano := hleft
      _ = (element' + (subtract index' index hlt) * diff).toPeano := hright
      _ = element'.toPeano + ((subtract index' index hlt) * diff).toPeano := by
            rw [add_toPeano]
            ...
            · simp only [hwalk, ↓reduceIte] at h
              ...
              have hget :=
                getElementFrom_of_tryFirst_tryCommonDifference
                  index1 element1 index2 element2 hlt diff first length
                  hlen_ne hwalk hle2 hd hf
              have hlenp :=
        ...
        | some diff =>
          ...
          | some first =>
            ...
            · simp only [hwalk, ↓reduceIte] at h
              ...
              have hget :=
                getElementFrom_of_tryFirst_tryCommonDifference
                  index2 element2 index1 element1 hgt diff first length
                  hlen_ne hwalk hle1 hd hf
              have hlenp :=
                getLength_lastElementFrom first diff length hlen_ne hwalk
              have hle1p :
                  CardinalNatural.Decimal.fromOrdinal index1 ≤
...
theorem agreesWithMaskedElementsFromCurrent_eq_true_iff
    (p : ArithmeticDecreasing) (index : Decimal) (current : Option Decimal)
    (elements : Sequences.List (Option Decimal))
    (hcur : current =
      Sequences.Progression.tryGetElement index.toPeano (toProgression p)) :
    agreesWithMaskedElementsFromCurrent p.subtractiveCommonDifference p.limit
...
theorem agreesWithMaskedElementsFrom_unmasked_eq_true
    (p : ArithmeticDecreasing) (index x : Decimal)
...
theorem getLength_of_trySubtract_multiply (first diff n last : Decimal)
    ...
    getLength {
      ...
      CardinalNatural.Decimal.fromOrdinal n.successor := by
  apply CardinalNatural.Decimal.equivalent_of_toPeano_eq
  have hstruct :
      toPeano {
        first := some first
  ...
  have hs_peano :
    ...
    exact hmap.symm
  have hlen :=
    Peano.Progressions.ArithmeticDecreasing.getLength_of_trySubtract_multiply
      first.toPeano diff.toPeano n.toPeano last.toPeano hs_peano
  rw [hlen, CardinalNatural.Decimal.fromOrdinal_toPeano_eq_fromOrdinal_peano,
    successor_toPeano]
  ...
  · next first hf =>
    ...
    · next hz =>
      exact (CardinalNatural.Peano.not_two_le_zero
        (((CardinalNatural.Decimal.toPeano_eq_of_equivalent
            (CardinalNatural.Decimal.eq_zero_of_le_zero _
              (CardinalNatural.Decimal.le_of_le_of_equivalent hleLen hz))).trans
          CardinalNatural.Decimal.toPeano_zero) ▸ hge)).elim
    · next hz =>
      split at h
      · next h1 =>
        exact (CardinalNatural.Peano.not_two_le_one (by
          have hgeLen : CardinalNatural.Peano.two ≤ length.toPeano :=
            CardinalNatural.Peano.le_trans hge
              (CardinalNatural.Decimal.toPeano_le_of_le hleLen)
          obtain ⟨hne_peano, hpred⟩ :=
            CardinalNatural.Decimal.predecessor_toPeano length hz
          have hn_one : length.toPeano = CardinalNatural.Peano.one := by
            have hsucc :=
              ...
              (fromPeano
                (CardinalNatural.Peano.toOrdinal
                  ...
                  (CardinalNatural.Decimal.toPeano_ne_zero_of_not_equivalent_zero
                    h1)))
              last hs
          refine Setoid.trans hlen ?_
          apply CardinalNatural.Decimal.equivalent_of_toPeano_eq
          rw [CardinalNatural.Decimal.fromOrdinal_toPeano_eq_fromOrdinal_peano,
            successor_toPeano, toPeano_fromPeano]
          have hsucc : (length.predecessor hz).successor = length :=
            CardinalNatural.Decimal.successor_predecessor length hz
          have hlen_toPeano :
              length.toPeano = (length.predecessor hz).toPeano.successor := by
            calc
              length.toPeano
                  = ((length.predecessor hz).successor).toPeano := by rw [hsucc]
                _ = (length.predecessor hz).toPeano.successor :=
                  CardinalNatural.Decimal.successor_toPeano _
          rw [hlen_toPeano]
          change
              (CardinalNatural.Peano.fromOrdinal
                  (CardinalNatural.Peano.toOrdinal
                    ...
                    (CardinalNatural.Decimal.toPeano_ne_zero_of_not_equivalent_zero
                      h1))).successor =
                (length.predecessor hz).toPeano.successor
...
theorem getElement_of_tryExtendToLength (p : ArithmeticDecreasing)
    ...
    (index : Decimal)
    (hle : CardinalNatural.Decimal.fromOrdinal index ≤ getLength p) :
    ∃ (hle' : CardinalNatural.Decimal.fromOrdinal index ≤ getLength q),
      getElement q index hle' = getElement p index hle := by
  have hlenq := getLength_of_tryExtendToLength p hge length hleLen q h
  have hle' :
      CardinalNatural.Decimal.fromOrdinal index ≤ getLength q :=
    CardinalNatural.Decimal.le_of_le_of_equivalent
      (CardinalNatural.Decimal.le_trans hle hleLen) (Setoid.symm hlenq)
  refine ⟨hle', ?_⟩
  match hf : effectiveFirst p with
  | none =>
    exact (CardinalNatural.Peano.not_two_le_zero
      ...
      · next first' hf' =>
        ...
        · next hz =>
          exact (CardinalNatural.Peano.not_two_le_zero
            (((CardinalNatural.Decimal.toPeano_eq_of_equivalent
                (CardinalNatural.Decimal.eq_zero_of_le_zero _
                  (CardinalNatural.Decimal.le_of_le_of_equivalent hleLen
                    hz))).trans
              CardinalNatural.Decimal.toPeano_zero) ▸ hge)).elim
        · next hz =>
          split at h
          · next h1 =>
            exact (CardinalNatural.Peano.not_two_le_one (by
              ...
              have hn_one : length.toPeano = CardinalNatural.Peano.one := by
                have hsucc :=
                  CardinalNatural.Peano.successor_predecessor length.toPeano
                    hne_peano
                have hpred0 :
                    (length.predecessor hz).toPeano =
