...
theorem toInt_positive (p : Peano) : p.toInt > 0 := by
  rw [toInt_eq_toNat]
...
@[simp]
theorem add_successor (a b : Peano) : a + successor b = successor (a + b) := rfl
...
theorem add_commutative (a b : Peano) : a + b = b + a := by
  ...
  | one =>
    rw [add_one, one_add]
  | successor b ih =>
    rw [add_successor, successor_add, ih]

theorem successor_lt_successor {x y : Peano} (h : x < y) : successor x < successor y := by
  induction h with
  | base => exact LessThan.base
  | step _ ih => exact LessThan.step ih

theorem successor_le_successor {a b : Peano} (h : a ≤ b) : successor a ≤ successor b := by
  cases h with
...
theorem lt_of_successor_lt {x y : Peano} (h : successor x < y) : x < y :=
  lt_trans LessThan.base h

theorem lt_of_successor_lt_successor {x y : Peano} (h : successor x < successor y) : x < y := by
...
def subtract (a : Peano) : (b : Peano) → b < a → Peano
  ...
  | successor b', h =>
    match a, h with
    | successor a', h' => subtract a' b' (lt_of_successor_lt_successor h')
...
theorem not_successor_le (a : Peano) : ¬ successor a ≤ a := by
  intro h
  cases h with
  | inl hlt => exact not_lt_of_lt LessThan.base hlt
  | inr heq => exact (ne_of_lt LessThan.base) heq.symm

theorem one_lt_successor (x : Peano) : one < successor x := by
  induction x with
...
theorem exists_subtract_of_trySubtract {x y z : Peano} (h : trySubtract x y = some z) :
  ...
  | successor y' ih =>
    ...
    | successor x' =>
      simp [trySubtract] at h
      obtain ⟨h', heq⟩ := ih h
      refine ⟨successor_lt_successor h', ?_⟩
...
theorem trySubtract_of_subtract {x y z : Peano} (h : ∃ h', subtract x y h' = z) :
  ...
  | successor y' ih =>
    ...
    | successor x' =>
      simp [trySubtract]
      apply ih
      refine ⟨lt_of_successor_lt_successor hlt, ?_⟩
      change subtract x' y' _ = z
      exact (subtract_eq_of_eq _ (lt_of_successor_lt_successor hlt) rfl rfl).symm.trans heq

theorem one_le (x : Peano) : x = one ∨ one < x := by
  induction x with
  | one => exact Or.inl rfl
  | successor x _ => exact Or.inr (one_lt_successor x)
...
theorem isLessThan_eq_true_iff_lt (a b : Peano) : Peano.isLessThan a b = true ↔ a < b := by
  ...
  | successor b ih =>
    ...
    | one =>
      simp [Peano.isLessThan]
      exact Peano.one_lt_successor b
    | successor a =>
      simp [Peano.isLessThan]
      rw [ih a]
      constructor
      · intro h
        exact Peano.successor_lt_successor h
      · intro h
        exact Peano.lt_of_successor_lt_successor h
...
theorem trichotomy_or (x y : Peano) : x < y ∨ x = y ∨ y < x := by
  ...
  | successor x ihx =>
    cases y with
    | one =>
      exact Or.inr (Or.inr (one_lt_successor x))
    | successor y =>
      cases ihx y with
...
def compare (a b : Peano) : Comparison a b :=
  ...
  | one, successor b => Comparison.less (one_lt_successor b)
  | successor a, one => Comparison.greater (one_lt_successor a)
...
theorem lt_add_right (x y : Peano) : y < x + y := by
  induction y with
  | one => exact one_lt_successor x
  | successor y ih => exact successor_lt_successor ih
...
theorem trySubtract_self_add (x d : Peano) : trySubtract (x + d) x = some d := by
  ...
  exact (subtract_eq_of_eq h' h (add_commutative x d) rfl).trans heq
...
theorem subtract_add_left (a b : Peano) :
  ...
  exact (subtract_eq_of_eq (lt_add_left a b) h (add_commutative a b) rfl).trans heq
...
theorem trySubtract_add_right (x d : Peano) : trySubtract (x + d) d = some x := by
  ...
  exact (subtract_eq_of_eq (lt_add_right x d) (lt_add_left d x) (add_commutative x d) rfl).trans
...
theorem subtract_add_cancel (a b : Peano) (h : b < a) : subtract a b h + b = a := by
  ...
  | successor b ih =>
    cases a with
    | one => cases not_lt_self _ (lt_trans h (one_lt_successor _))
    | successor a =>
      exact congrArg successor (ih a (lt_of_successor_lt_successor h))
...
theorem add_associative (a b c : Peano) : (a + b) + c = a + (b + c) := by
  induction c with
  | one =>
    rw [add_one, add_one, add_successor]
  | successor c ih =>
    rw [add_successor, add_successor, add_successor, ih]
...
theorem multiply_successor (a b : Peano) : a * successor b = a * b + a := by rfl
...
theorem successor_multiply (a b : Peano) : successor a * b = a * b + b := by
  ...
  | successor b ih =>
    ...
    rw [ih]
    rw [add_successor, add_successor]
    congr 1
    rw [add_right_commutative]
...
theorem multiply_commutative (a b : Peano) : a * b = b * a := by
  ...
  | successor b ih =>
    rw [multiply_successor, successor_multiply, ih]

theorem multiply_add (a b c : Peano) : a * (b + c) = a * b + a * c := by
  induction c with
  | one =>
    rw [add_one, multiply_successor, multiply_one]
  | successor c ih =>
    rw [add_successor, multiply_successor, ih, multiply_successor, add_associative]
...
theorem multiply_associative (a b c : Peano) : (a * b) * c = a * (b * c) := by
  ...
  | successor c ih =>
    rw [multiply_successor, multiply_successor, multiply_add, ih]
...
theorem successor_le_of_lt {a b : Peano} (h : a < b) : successor a ≤ b := by
  cases lt_successor_cases h with
  | inl heq => exact Or.inr heq.symm
  | inr hlt => exact Or.inl hlt

theorem le_of_lt_successor {a b : Peano} (h : a < successor b) : a ≤ b := by
...
theorem lt_of_successor_le {c b : Peano} (h : successor c ≤ b) : c < b := by
  cases h with
  | inl hlt =>
    cases b with
    | one => exact absurd hlt (not_lt_one (successor c))
    | successor b' => exact lt_trans (lt_of_successor_lt_successor hlt) LessThan.base
  | inr heq => subst heq; exact LessThan.base

theorem le_of_successor_le {c b : Peano} (h : successor c ≤ b) : c ≤ b := by
  cases h with
  | inl hlt =>
    cases b with
    | one => exact absurd hlt (not_lt_one (successor c))
    | successor b' =>
      exact Or.inl (lt_trans (lt_of_successor_lt_successor hlt) LessThan.base)
  | inr heq => subst heq; exact Or.inl LessThan.base

def divideWithRemainderAuxiliary (a b : Peano) (d : Option Peano) (c : Peano) (hc : c ≤ b) :
    Option Peano × Option Peano :=
  match a, d, c with
  | successor a, none, one =>
    divideWithRemainderAuxiliary a b (some one) b (Or.inr rfl)
  | successor a, some d, one =>
    divideWithRemainderAuxiliary a b (some d.successor) b (Or.inr rfl)
  | successor a, d, successor c =>
    divideWithRemainderAuxiliary a b d c (le_of_successor_le hc)
  | one, none, one =>
    (some one, none)
  | one, some d, one =>
    (some d.successor, none)
  | one, d, successor c =>
    (d, some (subtract b c (lt_of_successor_le hc)))

def divideWithRemainder (a b : Peano) : Option Peano × Option Peano :=
  divideWithRemainderAuxiliary a b none b (Or.inr rfl)
...
theorem subtract_lt_right (b c : Peano) (h : c < b) : subtract b c h < b := by
  ...
  | successor c ih =>
    ...
    | successor b' =>
      rw [subtract]
      exact lt_trans (ih b' (lt_of_successor_lt_successor h)) LessThan.base

theorem subtract_lt_of_successor_le {c b : Peano} (h : successor c ≤ b) :
    subtract b c (lt_of_successor_le h) < b :=
  subtract_lt_right b c (lt_of_successor_le h)

theorem divideWithRemainderAuxiliary_ne_none_none (a b : Peano) (d : Option Peano) (c : Peano)
    (hc : c ≤ b) : divideWithRemainderAuxiliary a b d c hc ≠ (none, none) := by
  induction a generalizing b d c with
  | one =>
    unfold divideWithRemainderAuxiliary
    cases d <;> cases c <;> simp
  | successor a ih =>
    unfold divideWithRemainderAuxiliary
    cases d <;> cases c <;> exact ih _ _ _ _

theorem divideWithRemainderAuxiliary_remainder_lt_b (a b : Peano) (d : Option Peano) (c : Peano)
    (hc : c ≤ b) (q : Option Peano) (r : Peano)
    (h : divideWithRemainderAuxiliary a b d c hc = (q, some r)) : r < b := by
  ...
  | one =>
    ...
    | none =>
      ...
      | successor c =>
        cases h
        exact subtract_lt_of_successor_le hc
    | some d =>
      cases c with
      | one => cases h
      | successor c =>
  ...
  | successor a ih =>
    ...
    | exact ih _ _ _ (Or.inr rfl) _ h
    | exact ih _ _ _ (le_of_successor_le hc) _ h
...
theorem add_cancel_right (a b c : Peano) (h : a + c = b + c) : a = b := by
  ...
  | one =>
    ...
    injection h
  | successor c ih =>
    rw [add_successor, add_successor] at h
    injection h with h'
    exact ih h'
...
theorem multiply_cancel_left (a b c : Peano) (h : a * b = a * c) : b = c := by
  ...
  | one =>
    ...
    | one => rfl
    | successor c' =>
      rw [multiply_one, multiply_successor] at h
  ...
  | successor b' ih =>
    cases c with
    | one =>
      rw [multiply_successor, multiply_one] at h
      have h1 : a < a * b' + a := lt_add_right (a * b') a
      rw [h] at h1
      cases not_lt_self a h1
    | successor c' =>
      rw [multiply_successor, multiply_successor] at h
...
theorem power_successor (a b : Peano) : a ^ successor b = a ^ b * a := rfl
...
theorem power_add (x y z : Peano) : x ^ (y + z) = (x ^ y) * (x ^ z) := by
  induction z with
  | one => rw [add_one, power_successor, power_one]
  | successor z ih =>
    rw [add_successor, power_successor, ih, power_successor]
    rw [multiply_associative]

theorem power_multiply (x y z : Peano) : x ^ (y * z) = (x ^ y) ^ z := by
  induction z with
  | one => rw [multiply_one, power_one]
  | successor z ih => simp [multiply_successor, power_add, power_successor, ih]
...
theorem multiply_power (x y z : Peano) : (x * y) ^ z = (x ^ z) * (y ^ z) := by
  ...
  | successor z ih =>
    rw [power_successor, power_successor, power_successor]
    rw [ih, multiply_associative, ← multiply_associative (y ^ z), multiply_commutative (y ^ z) x,
      multiply_associative x, ← multiply_associative (x ^ z)]
...
theorem add_lt_add_right {a b : Peano} (c : Peano) (h : a < b) : a + c < b + c := by
  ...
  | one =>
    ...
    exact successor_lt_successor h
  | successor c ih =>
...
theorem lt_multiply_left {a b c : Peano} (h : a < b) : a * c < b * c := by
  ...
  | one =>
    ...
    exact h
  | successor c ih =>
    rw [multiply_successor, multiply_successor]
    have h1 : a * c + a < b * c + a := add_lt_add_right a ih
    have h2 : b * c + a < b * c + b := by
      rw [add_commutative, add_commutative (b * c) b]
...
theorem multiply_subtract (a b c : Peano) (h : b > c) :
  ...
  have h2 : a * c < a * b := by
    ...
    rw [multiply_commutative c a] at hlt
    rw [multiply_commutative b a] at hlt
    exact hlt
  refine ⟨h2, ?_⟩
...
theorem subtract_subtract (x y z : Peano)
  ...
  have h3 : y + z < x := by
    ...
    rw [h_eq] at h_lt
    have h_comm : z + y = y + z := add_commutative z y
    rw [h_comm] at h_lt
    exact h_lt
  exact ⟨h3, by
...
theorem lt_power {a b e : Peano} (h : a < b) : a ^ e < b ^ e := by
  ...
  | one =>
    ...
    exact h
  | successor e ih =>
    rw [power_successor, power_successor]
    have h1 : a ^ e * a < b ^ e * a := lt_multiply_left ih
    have h2 : b ^ e * a < b ^ e * b := by
      rw [multiply_commutative, multiply_commutative (b ^ e) b]
...
theorem one_lt_two_power (e : Peano) : one < two ^ e := by
  induction e with
  | one =>
    simp [two, power_one]
    exact lt_add_right one one
  | successor e ih =>
    rw [power_successor, two, multiply_commutative]
...
theorem rootWithRemainderAuxiliary_reset_none_lt {b e : Peano} (hb : b = one) :
    b ^ e < two ^ e := by
  rw [hb, one_power e]
  exact one_lt_two_power e

theorem rootWithRemainderAuxiliary_one_none_successor_false {e c p b : Peano}
...
theorem rootWithRemainderAuxiliary_some_power_lt {k b e : Peano} (hb : b = k.successor) :
    k ^ e < b ^ e := by
  rw [hb]
  exact lt_power LessThan.base

theorem rootWithRemainderAuxiliary_advance_lt (b e : Peano) : b ^ e < b.successor ^ e :=
  lt_power LessThan.base

theorem rootWithRemainderAuxiliary_gap_lt_p {k p : Peano} (hlt : k < p) :
    subtract p k hlt < p :=
  subtract_lt_right p k hlt

theorem rootWithRemainderAuxiliary_c_lt_p {k c p e : Peano} (hlt : k ^ e < p)
    (hc : c ≤ subtract p (k ^ e) hlt) : c < p :=
  lt_of_le_lt hc (rootWithRemainderAuxiliary_gap_lt_p hlt)

def rootWithRemainderAuxiliary (a e : Peano) (r : Option Peano) (c p b : Peano)
  ...
  | successor a, none, one =>
    have hlt : p < two ^ e := by
      rw [hp]
      exact rootWithRemainderAuxiliary_reset_none_lt (hnone rfl)
    rootWithRemainderAuxiliary a e (some one) (subtract (two ^ e) p hlt) (two ^ e) two
  ...
  | successor a, some r, one =>
    ...
    have hlt : p < b.successor ^ e := by
      rw [hp]
      exact rootWithRemainderAuxiliary_advance_lt b e
    rootWithRemainderAuxiliary a e (some b) (subtract (b.successor ^ e) p hlt)
  ...
  | successor a, r, successor c =>
    rootWithRemainderAuxiliary a e r c p b (le_of_successor_le hc) hp hnone hsome
      (fun k hk hlt => le_of_successor_le (hbound k hk hlt))
  | one, none, one =>
    (one, none)
  | one, some r, one =>
    (r.successor, none)
  | one, none, successor c =>
    False.elim (rootWithRemainderAuxiliary_one_none_successor_false hc hp (hnone rfl))
  | one, some r, successor c =>
    have hlt_gap : r ^ e < p := by
      rw [hp, hsome r rfl]
      exact lt_power LessThan.base
    have hc_gap : successor c ≤ subtract p (r ^ e) hlt_gap := hbound r rfl hlt_gap
    have hlt_c : c < subtract p (r ^ e) hlt_gap := lt_of_successor_le hc_gap
    (r, some (subtract (subtract p (r ^ e) hlt_gap) c hlt_c))

def rootWithRemainder (a e : Peano) : Peano × Option Peano :=
  rootWithRemainderAuxiliary a e none one one one (Or.inr rfl) (by rw [one_power e])
...
theorem subtract_successor_add_one (b c : Peano) (hlt : successor c < b) :
    subtract b c (lt_of_successor_lt hlt) = one + subtract b (successor c) hlt := by
  induction c generalizing b with
  | one =>
    cases b with
...
theorem add_subtract_successor_step (a p c : Peano) (hlt : successor c < p) :
    ...
    a + subtract p c (lt_of_successor_lt hlt) := by
...
theorem subtract_subtract_cancel (x y : Peano) (h : y < x) :
  ...
  rw [subtract_add_cancel x (subtract x y h) h2, add_commutative, subtract_add_cancel x y h]
...
def rootWithRemainderOriginalSome (a c p : Peano) (hlt : c < p) : Peano :=
  a + subtract p c hlt

theorem rootWithRemainderAuxiliary_correct (e : Peano) :
    ∀ (a : Peano) (r : Option Peano) (c p b : Peano)
      (hc : c ≤ p) (hp : p = b ^ e) (hnone : r = none → b = one)
      (hsome : ∀ k, r = some k → b = k.successor)
      (hbound : ∀ k, r = some k → ∀ hlt : k ^ e < p, c ≤ subtract p (k ^ e) hlt),
      (r = none →
        (let res := rootWithRemainderAuxiliary a e r c p b hc hp hnone hsome hbound
         (res.2 = none → a = res.1 ^ e) ∧
         (∀ rem, res.2 = some rem → a = res.1 ^ e + rem ∧ a < res.1.successor ^ e))) ∧
      (∀ k, r = some k → ∀ (hlt : c < p),
        (let orig := rootWithRemainderOriginalSome a c p hlt
         let res := rootWithRemainderAuxiliary a e r c p b hc hp hnone hsome hbound
  ...
  | one =>
    ...
    · intro hr
      ...
      cases c with
      | one =>
        refine ⟨fun _ => by simp [rootWithRemainderAuxiliary, one_power], ?_⟩
        intro rem h
        simp [rootWithRemainderAuxiliary] at h
      | successor c =>
        exact False.elim (rootWithRemainderAuxiliary_one_none_successor_false hc hp (hnone rfl))
    · intro k hr hlt
      subst hr
      cases c with
      | one =>
        ...
        refine ⟨?_, ?_⟩
        · intro _
          simp only [rootWithRemainderAuxiliary, rootWithRemainderOriginalSome]
      ...
      | successor c =>
        ...
        have hlt_c_gap : c < subtract p (k ^ e) hlt_gap := lt_of_successor_le hc_gap
        have hlt_succ : successor c < p := rootWithRemainderAuxiliary_c_lt_p hlt_gap hc_gap
        refine ⟨?_, ?_⟩
        · intro h
          simp [rootWithRemainderAuxiliary] at h
        · intro rem hres
          have hrem : rem = subtract (subtract p (k ^ e) hlt_gap) c hlt_c_gap := by
            simp only [rootWithRemainderAuxiliary] at hres
            injection hres with hr
            exact hr.symm
          subst hrem
          constructor
          · simp only [rootWithRemainderOriginalSome, rootWithRemainderAuxiliary]
            ...
            have hone : one + subtract p (successor c) hlt_succ =
                subtract p c (lt_of_successor_lt hlt_succ) :=
              (subtract_successor_add_one p c hlt_succ).symm
            rw [hone]
            have hdecomp : subtract p c (lt_of_successor_lt hlt_succ) =
          ...
          · simp only [rootWithRemainderOriginalSome, rootWithRemainderAuxiliary]
            ...
            have hpc : subtract p (successor c) hlt =
                subtract p (successor c) hlt_succ :=
              subtract_eq_of_eq hlt hlt_succ rfl rfl
            rw [hpc, (subtract_successor_add_one p c hlt_succ).symm, ← hb, ← hp]
            exact subtract_lt_right p c (lt_of_successor_lt hlt_succ)
  | successor a ih =>
    intro r c p b hc hp hnone hsome hbound
    constructor
    · intro hr
      ...
      | one =>
        ...
        have horig_eq : rootWithRemainderOriginalSome a (subtract (two ^ e) p hlt) (two ^ e) hlt_c_new =
            successor a := by
          simp only [rootWithRemainderOriginalSome]
    ...
    · intro k hr hlt
      ...
      | one =>
        ...
        have hlt_adv : p < b.successor ^ e := by
          rw [hp]; exact rootWithRemainderAuxiliary_advance_lt b e
        ...
        have horig_eq : rootWithRemainderOriginalSome a (subtract (b.successor ^ e) p hlt_adv)
              (b.successor ^ e) hlt_c_new =
            rootWithRemainderOriginalSome (successor a) one p hlt := by
          simp only [rootWithRemainderOriginalSome]
          rcases subtract_subtract_cancel (b.successor ^ e) p hlt_adv with ⟨h2, hcancel⟩
          rw [subtract_eq_of_eq hlt_c_new h2 rfl rfl, hcancel]
          exact (successor_add_subtract_one a p hone_lt_p).symm
        simp only [rootWithRemainderAuxiliary]
        have ⟨ih_none, ih_rem⟩ := ih_at
        refine ⟨?_, ?_⟩
        · intro hnone_res
      ...
      | successor c =>
        have hbound' : ∀ k', some k = some k' →
          ...
          fun k' hk hlt' => le_of_successor_le (hbound k' hk hlt')
        have hc' : c ≤ p := le_of_successor_le hc
        have ih_pair := ih (some k) c p b hc' hp hnone hsome hbound'
        have ⟨_, ih_some⟩ := ih_pair
        have hlt' : c < p := lt_of_successor_lt hlt
        have ih_at := ih_some k rfl hlt'
        have horig_eq : rootWithRemainderOriginalSome a c p hlt' =
            rootWithRemainderOriginalSome (successor a) (successor c) p hlt := by
          simp only [rootWithRemainderOriginalSome]
          exact (add_subtract_successor_step a p c hlt).symm
        simp only [rootWithRemainderAuxiliary]
        have ⟨ih_none, ih_rem⟩ := ih_at
        refine ⟨?_, ?_⟩
        · intro hnone_res
...
theorem rootWithRemainder_none (a e b : Peano)
  ...
  have hcorr := (rootWithRemainderAuxiliary_correct e a none one one one
    (Or.inr rfl) (by rw [one_power e]) (fun _ => rfl)
    (fun _ hk => by cases hk) (fun _ hk => by cases hk)).1 rfl
  simp only [rootWithRemainder] at h
  have ⟨hnone, _⟩ := hcorr
  have hb : (rootWithRemainderAuxiliary a e none one one one (Or.inr rfl) (by rw [one_power e])
...
theorem rootWithRemainder_some_power (a e b r : Peano) (h : Power e a)
  ...
  exact not_lt_self b (lt_of_lt_le hbc (le_of_lt_successor hcs))
...
theorem root_interval_unique (a e b b' : Peano)
  ...
  | first hlt_bb' =>
    have hle_succ : b.successor ≤ b' := successor_le_of_lt hlt_bb'
  ...
  | third hlt_b'b =>
    have hle_succ : b'.successor ≤ b := successor_le_of_lt hlt_b'b
    have hle_a : b'.successor ^ e ≤ a :=
      le_trans (power_le_of_le hle_succ) hle
    exact False.elim (not_le_of_gt hlt' hle_a)
  ...
  | (b', some r') =>
    ...
    have hr : r' = r :=
      add_cancel_right r' r (b ^ e) (by
        rw [add_commutative r', add_commutative r]
        calc b ^ e + r'
            = b' ^ e + r' := by rw [heq]
          _ = a := hadd'.symm
          _ = b ^ e + r := hadd)
...
theorem toNat_multiply (a b : Peano) : (a * b).toNat = a.toNat * b.toNat := by
  ...
  | successor b ih =>
    rw [multiply_successor, toNat_add, ih]
    simp [toNat, Nat.mul_add]

@[simp]
theorem two_toNat : two.toNat = 2 := rfl

theorem predecessor_toNat {x : Peano} (h_neq : x ≠ one) : (predecessor x h_neq).toNat = x.toNat - 1 := by
  cases x with
  | one => contradiction
  | successor x' =>
...
theorem even_successor {x : Peano} : Even x → Odd (successor x) := by
  intro h
  ...
  | one =>
    ...
    | successor c' =>
      ...
      rw [add_commutative (two * c') two, add_commutative two one, add_commutative two (two * c')] at hc'_symm
  ...
  | successor c ih =>
    ...
    | one =>
      rw [multiply_one] at hc'_symm
      have h_succ_add : (two * c + two).successor = two * c + two.successor := by rw [add_successor]
      ...
      have hlt3 : two < two.successor := successor_lt_successor hlt2
    ...
    | successor c' =>
      ...
      have h_succ_add : (two * c + two).successor = (two * c).successor + two := by
        have h1 : (two * c + two).successor = two * c + two.successor := by rw [add_successor]
        rw [h1]
        have h2 : two.successor = one + two := by rw [one_add]
        rw [h2]
        have h3 : two * c + (one + two) = (two * c + one) + two := by rw [← add_associative]
        rw [h3]
...
theorem odd_successor {x : Peano} : Odd x → Even (successor x) := by
  intro h
  induction x with
  | one =>
  ...
  | successor x ih =>
    ...
    | inl h_even =>
      ...
      rw [show two = successor one from rfl, add_successor, add_one]
    | inr h_odd =>
      have h_even_succ : Even (successor x) := ih h_odd
      unfold Odd at h
      exact (h h_even_succ).elim

theorem even_predecessor {x : Peano} (h : Even x) : ∃ h_gt, Odd (predecessor x h_gt) := by
  have h_neq : x ≠ one := by
    intro h_eq
    rw [h_eq] at h
    unfold Even Divisible at h
  ...
  exists h_neq
  intro hcontra
  have h_even_succ : Even (predecessor x h_neq) → Odd (successor (predecessor x h_neq)) := even_successor
...
theorem odd_predecessor {x : Peano} (h_odd : Odd x) (h_neq : x ≠ one) : Even (predecessor x h_neq) := by
  have h_succ_pred : successor (predecessor x h_neq) = x := by
    cases x with
    | one => exact False.elim (h_neq rfl)
    | successor x' => rfl
  ...
  | inr h_odd_pred =>
    have h_even_succ : Even (successor (predecessor x h_neq)) := odd_successor h_odd_pred
...
theorem not_even_one : ¬ Even one := by
  ...
  | successor c' =>
    ...
    have hlt : one < two * c' + two := by
      ...
      have h3 : two + two * c' = two * c' + two := add_commutative two (two * c')
      rw [h3] at h2
      exact lt_trans h1 h2
    rw [hc] at hlt
    exact not_lt_self one hlt
...
theorem predecessor_successor_eq (x : Peano) : ∃ h, predecessor (successor x) h = x := by
  exact ⟨fun h => Peano.noConfusion h, rfl⟩

@[simp]
theorem successor_predecessor_eq (x : Peano) (h : x ≠ one) : successor (predecessor x h) = x := by
  cases x with
...
theorem x_lt_successor_x (x : Peano) : x < x.successor := by
  induction x with
  | one => exact one_lt_successor one
  | successor x' ih => exact successor_lt_successor ih
...
theorem subtractWithRemainder_correct (a b : Peano) :
  ...
  | one =>
    ...
    · cases one_le b with
      ...
      | inr h => exact Or.inl h
    · exact ⟨one_lt_successor b, rfl⟩
  | successor a ih =>
    cases b with
    | one =>
      apply Or.inr
      constructor
      · exact one_lt_successor a
      · exact ⟨one_lt_successor a, rfl⟩
    | successor b =>
      cases ih b with
      | inl h =>
        apply Or.inl
        ...
        · rcases h.right with ⟨h_lt, h_eq⟩
          ...
          exists successor_lt_successor h_lt
      | inr h =>
        apply Or.inr
        constructor
        · exact successor_lt_successor h.left
        · rcases h.right with ⟨h_lt, h_eq⟩
          unfold subtractWithRemainder
          exists successor_lt_successor h_lt
...
def divideWithRemainderOriginalNoneLt (b a c : Peano) (hlt : c < b) : Peano :=
  a + subtract b c hlt

def divideWithRemainderOriginalSomeBase (b : Peano) (hb : one < b) (q : Peano) : Peano :=
  match q with
  | one => subtract b one hb
  | successor q' => q' * b + subtract b one hb

def divideWithRemainderOriginalSomeLt (b : Peano) (hb : one < b) (q a c : Peano) (hlt : c < b) : Peano :=
  divideWithRemainderOriginalSomeBase b hb q + successor a + subtract b c hlt
...
theorem one_add_ne (x : Peano) : one + x ≠ x := by
  ...
  | successor x ih =>
    intro h
    rw [add_successor] at h
    injection h with h'
    exact ih h'

theorem subtract_successor_self (c : Peano) :
...
theorem not_successor_lt (c : Peano) : ¬ successor c < c := by
  intro h
  exact not_lt_self _ (lt_trans h LessThan.base)

theorem subtract_one_of_successor_eq (b c' : Peano) (heq : successor c' = b) :
    subtract b c' (lt_of_successor_le (Or.inr heq)) = one := by
  subst heq
  exact subtract_successor_self c'

def divideWithRemainderOriginalNone (b a c : Peano) (hc : c ≤ b) : Peano :=
  if h : c = b then a else
    divideWithRemainderOriginalNoneLt b a c (lt_of_le_of_ne hc h)

def divideWithRemainderOriginalSome (b : Peano) (hb : one < b) (q a c : Peano) (hc : c ≤ b) : Peano :=
  if h : c = b then
    divideWithRemainderOriginalSomeBase b hb q + successor a
  else
    divideWithRemainderOriginalSomeLt b hb q a c (lt_of_le_of_ne hc h)

theorem divideWithRemainderOriginal_optionSome (b : Peano) (q a c : Peano) (hc : c ≤ b) :
    (match some q with
      | none => divideWithRemainderOriginalNone b a c hc
      | some q' =>
        if hone : one < b then
          divideWithRemainderOriginalSome b hone q' a c hc
        else
          a + q') =
    if hone : one < b then
      divideWithRemainderOriginalSome b hone q a c hc
...
theorem successor_successor_add_balance (x y s : Peano) :
    (x + y).successor.successor + s = x + y + one.successor + s := by
  calc
    (x + y).successor.successor + s
        = ((x + y) + one).successor + s := by rw [add_one]
    _ = (x + y + one).successor + s := by rw [add_associative]
    _ = x + y + one.successor + s := by rw [add_successor, add_associative]

theorem successor_self_add_balance (s t : Peano) :
    s.successor.successor + t = s + one.successor + t := by
  calc
    s.successor.successor + t
        = (s + one).successor + t := by rw [add_one]
    _ = s + one.successor + t := by rw [←add_successor, add_associative]

theorem successor_self_eq (s : Peano) : s.successor.successor = s + one.successor := by
  simp [add_one, add_successor]

theorem add_one_one_split (s : Peano) :
    s + (one + one + s) = (s + one) + (one + s) := by
  simp [add_commutative]

theorem original_some_one_lt (b : Peano) (hb : one < b) :
    subtract b one hb + one.successor + subtract b one hb = b * one.successor := by
  let s := subtract b one hb
  calc
    s + one.successor + s
        = s + (one + one + s) := by rw [←add_one, add_associative]
    _ = (s + one) + (one + s) := add_one_one_split s
    _ = b + (one + s) := by rw [subtract_add_cancel b one hb]
    _ = b + (s + one) := by rw [add_commutative one s]
    _ = b + b := by rw [subtract_add_cancel b one hb]
    _ = b * one.successor := by
          apply Eq.symm
          rw [multiply_successor, multiply_one]

theorem original_some_one_lt' (b : Peano) (hb : one < b) (x : Peano) :
    x + subtract b one hb + one.successor + subtract b one hb = x + b * one.successor := by
  let s := subtract b one hb
  calc x + s + one.successor + s
      = x + (s + one.successor + s) := by simp [add_associative]
    _ = x + b * one.successor := by rw [original_some_one_lt b hb]

theorem original_some_one_step (b : Peano) (hb : one < b) :
    subtract b one hb + one.successor = b + one := by
  calc subtract b one hb + one.successor
      = subtract b one hb + (one + one) := by rw [←add_one one]
    _ = (subtract b one hb + one) + one := by rw [add_associative]
    _ = b + one := by rw [subtract_add_cancel b one hb]

theorem original_some_one_step' (b : Peano) (hb : one < b) (x r : Peano) :
    x + subtract b one hb + one.successor + r = x + (b + one + r) := by
  let s := subtract b one hb
  calc x + s + one.successor + r
      = x + (s + one.successor) + r := by rw [←add_associative]
    _ = x + (b + one) + r := by rw [original_some_one_step b hb, ←add_associative]
    _ = x + (b + one + r) := by rw [add_associative]

theorem original_some_q_multiply (b : Peano) (q' c' : Peano)
    (hlt' : successor c' < b) :
    q' * b + (b + one + subtract b (successor c') hlt') =
    b * (successor q') + subtract b c' (lt_of_successor_le (Or.inl hlt')) := by
  let rem := subtract b (successor c') hlt'
  have hsub := subtract_successor_add_one b c' hlt'
  calc q' * b + (b + one + rem)
      = (q' * b + (b + one)) + rem := by rw [←add_associative]
    _ = ((q' * b + b) + one) + rem := by rw [←add_associative]
    _ = ((b * q' + b) + one) + rem := by rw [multiply_commutative]
    _ = (b * (successor q') + one) + rem := by rw [←multiply_successor, add_associative]
    _ = b * (successor q') + (one + rem) := by rw [add_associative]
...
theorem divideWithRemainderOriginalNoneLt_step (b a c' : Peano) (hlt : successor c' < b) :
    ...
    divideWithRemainderOriginalNoneLt b a c' (lt_of_successor_lt hlt) := by
  unfold divideWithRemainderOriginalNoneLt
  calc
    successor a + subtract b (successor c') hlt
        = a + one + subtract b (successor c') hlt := by rw [←add_one, add_associative]
    _ = a + subtract b c' (lt_of_successor_lt hlt) := by
          rw [add_associative, (subtract_successor_add_one b c' hlt).symm]

theorem divideWithRemainderOriginalNone_ne (b a c : Peano) (hc : c ≤ b) (hne : c ≠ b) :
    divideWithRemainderOriginalNone b a c hc =
    divideWithRemainderOriginalNoneLt b a c (lt_of_le_of_ne hc hne) := by
...
theorem divideWithRemainderOriginalSome_ne (b : Peano) (hb : one < b) (q a c : Peano) (hc : c ≤ b)
    (hne : c ≠ b) :
    divideWithRemainderOriginalSome b hb q a c hc =
    divideWithRemainderOriginalSomeLt b hb q a c (lt_of_le_of_ne hc hne) := by
...
theorem divideWithRemainderOriginalSome_eq (b : Peano) (hb : one < b) (q a c : Peano) (hc : c = b) :
    divideWithRemainderOriginalSome b hb q a c (Or.inr hc) =
    divideWithRemainderOriginalSomeBase b hb q + successor a := by
  unfold divideWithRemainderOriginalSome divideWithRemainderOriginalSomeBase
  simp [hc]

theorem divideWithRemainderOriginalNone_step (b : Peano) (a c' : Peano)
    (hc : successor c' ≤ b) :
    divideWithRemainderOriginalNone b (successor a) (successor c') hc =
    divideWithRemainderOriginalNone b a c' (le_of_successor_le hc) := by
  ...
  · cases hc with
    ...
    | inr heq =>
      ...
      have hsub : subtract b c' (lt_of_successor_le (Or.inr heq)) = one := by
        simpa [heq.symm] using subtract_successor_self c'
      calc
        divideWithRemainderOriginalNone b (successor a) (successor c') (Or.inr heq)
            = successor a := divideWithRemainderOriginalNone_eq _ _ _ heq
        _ = divideWithRemainderOriginalNoneLt b a c' (lt_of_successor_le (Or.inr heq)) := by
              rw [divideWithRemainderOriginalNoneLt, hsub, add_one]
        _ = divideWithRemainderOriginalNone b a c' (le_of_successor_le (Or.inr heq)) :=
              (divideWithRemainderOriginalNone_ne _ _ _ _ hne).symm
  · have hlt' := lt_of_le_of_ne hc hcb
    have hne' : c' ≠ b := ne_of_lt (lt_of_successor_le hc)
    calc
      divideWithRemainderOriginalNone b (successor a) (successor c') hc
          = divideWithRemainderOriginalNoneLt b (successor a) (successor c') hlt' := by
            exact divideWithRemainderOriginalNone_ne _ _ _ _ hcb
      _ = divideWithRemainderOriginalNoneLt b a c' (lt_of_successor_lt hlt') :=
            divideWithRemainderOriginalNoneLt_step b a c' hlt'
      _ = divideWithRemainderOriginalNone b a c' (le_of_successor_le hc) := by
            exact (divideWithRemainderOriginalNone_ne _ _ _ _ hne').symm

theorem divideWithRemainderOriginalNone_reset (b : Peano) (hb : one < b) (a : Peano) :
    divideWithRemainderOriginalNone b (successor a) one (Or.inl hb) =
    divideWithRemainderOriginalSome b hb one a b (Or.inr rfl) := by
  have hone_ne : one ≠ b := ne_of_lt hb
  simp [divideWithRemainderOriginalNone, divideWithRemainderOriginalSome,
    divideWithRemainderOriginalNoneLt, divideWithRemainderOriginalSomeBase, hone_ne]
  rw [add_commutative]

theorem divideWithRemainderOriginalSomeLt_step (b : Peano) (hb : one < b) (q a c' : Peano)
    (hlt : successor c' < b) :
    divideWithRemainderOriginalSomeLt b hb q (successor a) (successor c') hlt =
    divideWithRemainderOriginalSomeLt b hb q a c' (lt_of_successor_lt hlt) := by
  unfold divideWithRemainderOriginalSomeLt divideWithRemainderOriginalSomeBase
  calc
    divideWithRemainderOriginalSomeBase b hb q + successor (successor a) +
        subtract b (successor c') hlt
        = divideWithRemainderOriginalSomeBase b hb q +
            (successor (successor a) + subtract b (successor c') hlt) := by rw [←add_associative]
    _ = divideWithRemainderOriginalSomeBase b hb q +
            (successor a + subtract b c' (lt_of_successor_lt hlt)) := by
          congr 1
          exact divideWithRemainderOriginalNoneLt_step b (successor a) c' hlt
    _ = divideWithRemainderOriginalSomeBase b hb q + successor a +
            subtract b c' (lt_of_successor_lt hlt) := by rw [add_associative]

theorem divideWithRemainderOriginalSome_step (b : Peano) (hb : one < b) (q a c' : Peano)
    (hc : successor c' ≤ b) :
    divideWithRemainderOriginalSome b hb q (successor a) (successor c') hc =
    divideWithRemainderOriginalSome b hb q a c' (le_of_successor_le hc) := by
  by_cases hcb : successor c' = b
  · cases hc with
    | inl hlt =>
      rw [hcb] at hlt
    ...
    | inr heq =>
      ...
      have hsub : subtract b c' (lt_of_successor_le (Or.inr heq)) = one := by
        simpa [heq.symm] using subtract_successor_self c'
      calc
        divideWithRemainderOriginalSome b hb q (successor a) (successor c') (Or.inr heq)
            = divideWithRemainderOriginalSomeBase b hb q + successor (successor a) :=
              divideWithRemainderOriginalSome_eq _ _ _ (successor a) (successor c') heq
        _ = divideWithRemainderOriginalSomeLt b hb q a c' (lt_of_successor_le (Or.inr heq)) := by
              simp [divideWithRemainderOriginalSomeLt, divideWithRemainderOriginalSomeBase,
                hsub, add_one]
        _ = divideWithRemainderOriginalSome b hb q a c' (le_of_successor_le (Or.inr heq)) :=
              (divideWithRemainderOriginalSome_ne _ _ _ _ _ _ hne).symm
  · have hlt' := lt_of_le_of_ne hc hcb
    have hne' : c' ≠ b := ne_of_lt (lt_of_successor_le hc)
    calc
      divideWithRemainderOriginalSome b hb q (successor a) (successor c') hc
          = divideWithRemainderOriginalSomeLt b hb q (successor a) (successor c') hlt' := by
            exact divideWithRemainderOriginalSome_ne _ _ _ _ _ _ hcb
      _ = divideWithRemainderOriginalSomeLt b hb q a c' (lt_of_successor_lt hlt') :=
            divideWithRemainderOriginalSomeLt_step b hb q a c' hlt'
      _ = divideWithRemainderOriginalSome b hb q a c' (le_of_successor_le hc) := by
            exact (divideWithRemainderOriginalSome_ne _ _ _ _ _ _ hne').symm

theorem divideWithRemainderOriginalSome_reset_auxiliary (b a : Peano) (hb : one < b) :
  ...
  calc
    ...
    _ = a + (s + s + one) := by rw [add_associative]
    _ = a + (s + b) := by
          congr 1
          rw [←subtract_add_cancel b one hb, add_associative]
    _ = a + (b + s) := by congr 1; exact add_commutative s b

theorem double_subtract_one (b : Peano) (hb : one < b) :
    subtract b one hb + subtract b one hb + one = b + subtract b one hb := by
  let s := subtract b one hb
  have h := divideWithRemainderOriginalSome_reset_auxiliary b one hb
  have h' : one + (s + s + one) = one + (b + s) := by
    calc
      one + (s + s + one)
          = (one + (s + s)) + one := by rw [←add_associative]
      _ = one + (b + s) := by simpa [←add_one] using h
  exact add_cancel_right (s + s + one) (b + s) one (by
    rw [←add_commutative one (s + s + one), ←add_commutative one (b + s)]
    exact h')

theorem subtract_successor_successor_balance (b a : Peano) (hb : one < b) :
    subtract b one hb + successor (successor a) + subtract b one hb =
    b + subtract b one hb + successor a := by
  let s := subtract b one hb
  have hcancel := double_subtract_one b hb
  calc
    s + successor (successor a) + s
        = s + (s + successor (successor a)) := by
          rw [add_commutative (s + successor (successor a))]
    _ = s + s + (successor a + one) := by
          rw [add_associative, ←add_one (successor a)]
    _ = (s + s + one) + successor a := by
          rw [add_associative, add_associative, add_commutative one (successor a), add_associative]
    _ = (s + s + one) + successor a := by rw [add_associative, add_associative]
    _ = (b + s) + successor a := by rw [hcancel]
    _ = b + s + successor a := by rw [add_associative]

theorem add_q_original_shift (q' b s a : Peano) :
...
theorem fin_q_balance (q' b s a : Peano) :
  ...
  calc
    b + (successor a + q' * b + s)
        = (b + (successor a + q' * b)) + s := by rw [←add_associative]
    _ = ((successor a + q' * b) + b) + s := by rw [add_commutative b (successor a + q' * b), add_associative]
    _ = (successor a + q' * b) + (b + s) := by rw [add_associative]
    _ = successor a + (q' * b + (b + s)) := by rw [add_associative, ←add_associative]
    _ = successor a + ((q' * b + b) + s) := by congr 1; rw [add_associative]
    _ = ((q' * b + b) + s) + successor a := by rw [add_commutative (successor a) ((q' * b + b) + s)]

theorem divideWithRemainderOriginalSome_reset_mid (b a : Peano) (hb : one < b) :
  ...
  calc
    s + (successor a + (s + s)).successor
        = s + (successor a + (b + s)) := by
          rw [divideWithRemainderOriginalSome_reset_auxiliary b (successor a) hb]
    _ = s + successor a + (b + s) := by rw [add_associative]

theorem divideWithRemainderOriginalSome_reset (b : Peano) (hb : one < b) (q a : Peano) :
    divideWithRemainderOriginalSome b hb q (successor a) one (Or.inl hb) =
    divideWithRemainderOriginalSome b hb (successor q) a b (Or.inr rfl) := by
  have hone_ne : one ≠ b := ne_of_lt hb
  let s := subtract b one hb
  cases q with
  | one =>
    calc
      divideWithRemainderOriginalSome b hb one (successor a) one (Or.inl hb)
          = divideWithRemainderOriginalSomeLt b hb one (successor a) one hb :=
            divideWithRemainderOriginalSome_ne _ _ _ _ _ _ hone_ne
      _ = s + successor (successor a) + s := by
            unfold divideWithRemainderOriginalSomeLt divideWithRemainderOriginalSomeBase
            rw [add_associative, add_commutative s]
      _ = b + s + successor a := subtract_successor_successor_balance b a hb
      _ = divideWithRemainderOriginalSomeBase b hb (successor one) + successor a := by
            dsimp [divideWithRemainderOriginalSomeBase]
            rw [one_multiply b]
      _ = divideWithRemainderOriginalSome b hb (successor one) a b (Or.inr rfl) :=
            (divideWithRemainderOriginalSome_eq b hb (successor one) a b rfl).symm
  | successor q' =>
    calc
      divideWithRemainderOriginalSome b hb (successor q') (successor a) one (Or.inl hb)
          = divideWithRemainderOriginalSomeLt b hb (successor q') (successor a) one hb :=
            divideWithRemainderOriginalSome_ne _ _ _ _ _ _ hone_ne
      _ = (q' * b + s) + successor (successor a) + s := by
            unfold divideWithRemainderOriginalSomeLt divideWithRemainderOriginalSomeBase
            rw [add_associative, add_commutative (q' * b + s)]
      _ = (q' * b + b + s) + successor a := by
        calc
          (q' * b + s) + successor (successor a) + s
              = q' * b + (s + successor (successor a) + s) := add_q_original_shift q' b s a
          _ = q' * b + (b + s + successor a) := by
                congr 1
                exact subtract_successor_successor_balance b a hb
          _ = (q' * b + b + s) + successor a := by
            calc
              q' * b + (b + s + successor a)
                  = (q' * b + (b + s)) + successor a :=
                    (add_associative (q' * b) (b + s) (successor a)).symm
              _ = ((q' * b + b) + s) + successor a := by
                    congr 1
                    exact (add_associative (q' * b) b s).symm
      _ = (successor q' * b + s) + successor a := by
            rw [←successor_multiply q' b, add_associative]
      _ = divideWithRemainderOriginalSomeBase b hb (successor (successor q')) + successor a := by
            unfold divideWithRemainderOriginalSomeBase
            rfl
      _ = divideWithRemainderOriginalSome b hb (successor (successor q')) a b (Or.inr rfl) :=
            (divideWithRemainderOriginalSome_eq b hb (successor (successor q')) a b rfl).symm

theorem divideWithRemainderAuxiliary_correct (b : Peano) :
    ∀ (a : Peano) (d : Option Peano) (c : Peano) (hc : c ≤ b),
      let res := divideWithRemainderAuxiliary a b d c hc
  ...
  | one =>
    ...
    | none =>
      ...
      | one =>
        constructor
        · exact divideWithRemainderAuxiliary_ne_none_none one b none one hc
        · constructor
          · intro r hr; cases hr
          · constructor
            ...
            · constructor
              · intro q hRes
                ...
                | one =>
                  ...
                  | inr heq => simp [divideWithRemainderOriginalNone, multiply_one]
                | successor b' =>
                  let b := successor b'
                  have hb := one_lt_successor b'
                  cases hc with
                  | inl hlt =>
                    simp [divideWithRemainderOriginalNone, divideWithRemainderOriginalNoneLt, multiply_one]
                    calc
                      one + b.subtract one hlt = b.subtract one hlt + one := add_commutative _ _
                      _ = successor b' := (subtract_add_cancel b one hlt).symm
                  | inr heq => exact absurd heq (fun h => by cases h)
              · intro q r hRes; cases hRes
      | successor c' =>
        have hlt := subtract_lt_of_successor_le hc
        cases hc with
        | inl hlt' =>
          constructor
          · exact divideWithRemainderAuxiliary_ne_none_none one b none (successor c') (Or.inl hlt')
          · constructor
            ...
            · constructor
              · intro r hRes
                ...
                rw [divideWithRemainderOriginalNone_ne _ _ _ _ hne, divideWithRemainderOriginalNoneLt,
                  subtract_successor_add_one b c' hlt']
              · constructor
                · intro q hRes; cases hRes
                · intro q r hRes; cases hRes
        | inr heq =>
          constructor
          · exact divideWithRemainderAuxiliary_ne_none_none one b none (successor c') (Or.inr heq)
          · constructor
            · intro r hr; cases hr; exact subtract_lt_of_successor_le (Or.inr heq)
            · constructor
              · intro r hRes
                cases hRes
                simp [divideWithRemainderOriginalNone, heq, subtract_one_of_successor_eq b c' heq]
              · constructor
                · intro q hRes; cases hRes
                · intro q r hRes; cases hRes
    | some d =>
      ...
      | one =>
        ...
        | one =>
          constructor
          · exact divideWithRemainderAuxiliary_ne_none_none one one (some d) one hc
          · constructor
            · intro r hr; cases hr
            · constructor
              · intro r hRes; cases hRes
              · constructor
                · intro q hRes
                  ...
                  | inr heq =>
                    ...
                    simp [honef, add_one, add_commutative]
                · intro q r hRes; cases hRes
        | successor b' =>
          let b := successor b'
          have hb := one_lt_successor b'
          constructor
          · exact divideWithRemainderAuxiliary_ne_none_none one b (some d) one hc
          · constructor
            · intro r hr; cases hr
            · constructor
              · intro r hRes; cases hRes
              · constructor
                · intro q hRes
                  ...
                  | one =>
                    ...
                    rw [divideWithRemainderOriginal_optionSome b one one one hc]
                    by_cases hone : one < b
                    · rw [if_lt_pos_named b hone
                        (divideWithRemainderOriginalSome b hone one one one hc) (one + one),
                      divideWithRemainderOriginalSome_ne b hone one one one hc hcone]
                      dsimp [divideWithRemainderOriginalSomeLt, divideWithRemainderOriginalSomeBase]
                      rw [successor_self_add_balance (subtract b one hone) (subtract b one hone),
                        original_some_one_lt b hone]
                    · exact absurd hb hone
                  | successor d' =>
                    have hcone : one ≠ b := ne_of_lt hb
                    rw [divideWithRemainderOriginal_optionSome b (successor d') one one hc]
                    by_cases hone : one < b
                    · rw [if_lt_pos_named b hone
                        (divideWithRemainderOriginalSome b hone (successor d') one one hc) (one + successor d'),
                      divideWithRemainderOriginalSome_ne b hone (successor d') one one hc hcone]
                      dsimp [divideWithRemainderOriginalSomeLt, divideWithRemainderOriginalSomeBase]
                      calc (d' * b + subtract b one hone).successor.successor
                          + subtract b one hone
                          = d' * b + subtract b one hone + one.successor + subtract b one hone := by
                            rw [successor_self_add_balance (d' * b + subtract b one hone)
                              (subtract b one hone)]
                        _ = d' * b + b * one.successor := original_some_one_lt' b hone (d' * b)
                        _ = b * successor (successor d') := by
                              calc d' * b + (b + b)
                                  = (d' * b + b) + b := by rw [add_associative]
                                _ = (b * d' + b) + b := by rw [multiply_commutative]
                                _ = b * (successor d') + b := by rw [←multiply_successor]
                                _ = b * (successor (successor d')) := by rw [←multiply_successor]
                    · exact absurd hb hone
                · intro q r hRes; cases hRes
      | successor c' =>
        have hlt := subtract_lt_of_successor_le hc
        ...
        | inl hlt' =>
          ...
          | successor b' =>
            ...
            have hb := one_lt_successor b'
            constructor
            · exact divideWithRemainderAuxiliary_ne_none_none one b (some d) (successor c') (Or.inl hlt')
            · constructor
              · intro r hr; cases hr; exact hlt
              · constructor
                · intro r hRes; cases hRes
                · constructor
                  ...
                  · intro q r hRes
                    ...
                    rw [divideWithRemainderOriginal_optionSome b d one (successor c') (Or.inl hlt')]
                    by_cases hone : one < b
                    · rw [if_lt_pos_named b hone
                        (divideWithRemainderOriginalSome b hone d one (successor c') (Or.inl hlt')) (one + d),
                      divideWithRemainderOriginalSome_ne b hone d one (successor c') (Or.inl hlt') hne]
                      dsimp [divideWithRemainderOriginalSomeLt, divideWithRemainderOriginalSomeBase]
                      cases d with
                      | one =>
                        rw [successor_self_add_balance (subtract b one hone)
                            (subtract b (successor c') hlt'),
                          original_some_one_step b hone, add_associative, ←subtract_successor_add_one b c' hlt',
                          multiply_one]
                      | successor q' =>
                        calc (q' * b + subtract b one hone).successor.successor
                            ...
                            = q' * b + subtract b one hone + one.successor
                              ...
                              rw [successor_self_add_balance (q' * b + subtract b one hone)
                                (subtract b (successor c') hlt')]
                          _ = q' * b + (b + one + subtract b (successor c') hlt') := by
                              exact original_some_one_step' b hone (q' * b) (subtract b (successor c') hlt')
                          _ = b * (successor q') + subtract b c' (lt_of_successor_le (Or.inl hlt')) := by
                              exact original_some_q_multiply b q' c' hlt'
                    · exact absurd hb hone
        | inr heq =>
          cases b with
          ...
          | successor b' =>
            ...
            have hb := one_lt_successor b'
            constructor
            · exact divideWithRemainderAuxiliary_ne_none_none one b (some d) (successor c') (Or.inr heq)
            · constructor
              · intro r hr; cases hr; exact subtract_lt_of_successor_le (Or.inr heq)
              · constructor
                · intro r hRes; cases hRes
                · constructor
                  ...
                  · intro q r hRes
                    cases hRes
                    rw [divideWithRemainderOriginal_optionSome b d one (successor c') (Or.inr heq)]
                    by_cases hone : one < b
                    · rw [if_lt_pos_named b hone
                        (divideWithRemainderOriginalSome b hone d one (successor c') (Or.inr heq)) (one + d),
                      divideWithRemainderOriginalSome_eq b hone d one (successor c') heq]
                      dsimp [divideWithRemainderOriginalSomeBase, subtract_one_of_successor_eq b c' heq, add_one]
                      cases d with
                      | one =>
                        rw [successor_self_eq (subtract b one hone), original_some_one_step b hone, multiply_one,
                          subtract_one_of_successor_eq b c' heq]
                      | successor d' =>
                        have hrem := subtract_one_of_successor_eq b c' heq
                        calc (d' * b + subtract b one hone).successor.successor
                            = d' * b + subtract b one hone + one.successor := by
                              rw [successor_self_eq (d' * b + subtract b one hone)]
                          _ = d' * b + (b + one) := by
                              calc d' * b + subtract b one hone + one.successor
                                  = d' * b + (subtract b one hone + one.successor) := by rw [←add_associative]
                                _ = d' * b + (b + one) := by rw [original_some_one_step b hone]
                          _ = b * (successor d') + subtract b c' (lt_of_successor_le (Or.inr heq)) := by
                              calc d' * b + (b + one)
                                  = (d' * b + b) + one := by rw [←add_associative]
                                _ = (b * d' + b) + one := by rw [multiply_commutative]
                                _ = b * (successor d') + one := by rw [←multiply_successor]
                                _ = b * (successor d') + subtract b c' (lt_of_successor_le (Or.inr heq)) := by
                                      rw [hrem]
                    · exact absurd hb hone
  | successor a ih =>
    ...
    | none =>
      ...
      | one =>
        ...
        | successor b' =>
          ...
          have horig :=
            divideWithRemainderOriginalNone_reset b hb a
          refine ⟨hspec.1, hspec.2.1, ?_, ?_, ?_⟩
      ...
      | successor c' =>
        have hspec := ih none c' (le_of_successor_le hc)
        have horig :=
          divideWithRemainderOriginalNone_step b a c' hc
        refine ⟨hspec.1, hspec.2.1, ?_, ?_, ?_⟩
        · intro r hRes; exact horig.trans (hspec.2.2.1 hRes)
        · intro q hRes; exact horig.trans (hspec.2.2.2.1 hRes)
        · intro q r hRes; exact horig.trans (hspec.2.2.2.2 hRes)
    | some d =>
      ...
      | one =>
        ...
        | one =>
          ...
          have horig : successor a + d = a + d.successor := by
            simp [add_successor, add_commutative]
          refine ⟨hspec.1, hspec.2.1, ?_, ?_, ?_⟩
          · intro r hRes; exact horig.trans (hspec.2.2.1 hRes)
          · intro q hRes; exact horig.trans (hspec.2.2.2.1 hRes)
          · intro q r hRes; exact horig.trans (hspec.2.2.2.2 hRes)
        | successor b' =>
          ...
          have horig :=
            divideWithRemainderOriginalSome_reset b hb d a
          refine ⟨hspec.1, hspec.2.1, ?_, ?_, ?_⟩
          · intro r hRes; exact horig.trans (hspec.2.2.1 hRes)
          · intro q hRes; exact horig.trans (hspec.2.2.2.1 hRes)
          · intro q r hRes; exact horig.trans (hspec.2.2.2.2 hRes)
      | successor c' =>
        ...
        | successor b' =>
          let b := successor b'
          have hb := one_lt_successor b'
          have hspec := ih (some d) c' (le_of_successor_le hc)
          have horig :=
            divideWithRemainderOriginalSome_step b hb d a c' hc
          refine ⟨hspec.1, hspec.2.1, ?_, ?_, ?_⟩
          · intro r hRes; exact horig.trans (hspec.2.2.1 hRes)
          · intro q hRes; exact horig.trans (hspec.2.2.2.1 hRes)
          · intro q r hRes; exact horig.trans (hspec.2.2.2.2 hRes)
...
theorem divideWithRemainder_not_none_none (a b : Peano) :
  ...
  intro h
  have hspec := divideWithRemainderAuxiliary_correct b a none b (Or.inr rfl)
...
theorem divideWithRemainder_remainder_lt_b (a b : Peano) (q : Option Peano) (r : Peano)
  ...
  rw [divideWithRemainder] at h
  exact divideWithRemainderAuxiliary_remainder_lt_b a b none b (Or.inr rfl) q r h

theorem divideWithRemainder_none_some (a b : Peano) (r : Peano)
    (h : divideWithRemainder a b = (none, some r)) : a = r := by
  have hspec := divideWithRemainderAuxiliary_correct b a none b (Or.inr rfl)
  have horig : divideWithRemainderOriginalNone b a b (Or.inr rfl) = a := by
    simp [divideWithRemainderOriginalNone]
  rw [divideWithRemainder] at h
  have := hspec.2.2.1 h
...
mutual
  theorem not_mult_remainder_eq (b q r c' : Peano) (hlt : r < b)
    ...
    | one =>
      ...
      | successor c'' =>
        rw [multiply_successor, add_commutative] at h
        ...
        | successor c''' =>
          ...
          rw [multiply_successor, add_commutative] at hlt
    ...
    | successor q' ih =>
      ...
      | one =>
        ...
        | successor q'' =>
          ...
          rw [multiply_successor, add_commutative] at hzl
          rw [add_commutative r] at hzl
    ...
    | successor q' =>
      ...
      | one =>
        ...
        | successor q'' =>
          ...
          have hchain' : b < b * (successor q'') + r := by
            simpa [multiply_successor, add_commutative] using hchain
          rw [hrest] at hchain'
      ...
      | successor c'' =>
        ...
        have hmain : b * q' + r = b * (successor c'') := by
          calc b * q' + r
              _ = b + b * c'' := hrest
              _ = b * (successor c'') := by rw [multiply_successor, add_commutative]
        exact not_mult_remainder_eq b q' r (successor c'') hlt hmain
end

theorem multiply_ge_self (b q : Peano) : b ≤ b * q := by
  cases q with
  | one => exact Or.inr (multiply_one b).symm
  | successor q' =>
    exact Or.inl (by
      rw [multiply_successor, add_commutative]
      exact lt_add_left b (b * q'))

theorem multiply_add_ge_self (b q r : Peano) : b ≤ b * q + r := by
  cases multiply_ge_self b q with
  | inl hlt => exact Or.inl (lt_trans hlt (lt_add_left (b * q) r))
  | inr heq =>
    rw [← heq]
    exact Or.inl (lt_add_left b r)

theorem divide_remainder_unique (b q r q' r' : Peano) (hr : r < b) (hr' : r' < b)
    ...
    | successor q'' =>
      have hside : b * q + r + b = b * q'' + r' + b := by
        calc b * q + r + b
            ...
            _ = b * successor q + r := by rw [multiply_successor]
            _ = b * successor q'' + r' := h
            _ = (b * q'' + b) + r' := by rw [multiply_successor]
...
theorem divideWithRemainder_eq_of_none_some (a b r : Peano) (hlt : a < b)
  ...
  | (some q, some r') =>
    ...
    cases multiply_add_ge_self b q r' with
    | inl hlt_ba =>
...
theorem divideWithRemainder_eq_of_some_some (a b q r : Peano) (hlt : r < b)
  ...
  | (some q', some r') =>
    have ha' := divideWithRemainder_some_some a b q' r' hres
    have hlt' := divideWithRemainder_remainder_lt_b a b (some q') r' hres
    obtain ⟨hq, hr⟩ := divide_remainder_unique b q r q' r' hlt hlt' (ha.symm.trans ha')
...
theorem divideWithRemainder_add_right (a b : Peano) :
  ...
  | (none, some r) =>
    ...
    have hab : a + b = b * one + r := by
      rw [har, multiply_one, add_commutative]
...
theorem divideWithRemainder_none_some_divisible (a b : Peano) (r : Peano) (h : Divisible a b)
  ...
  | inr hc_lt =>
    ...
    | successor c' =>
      rw [multiply_successor, add_commutative] at hc
      have hgt : b < b + b * c' := lt_add_left b (b * c')
      rw [hc] at hgt
      exact not_lt_of_lt hlt hgt
...
theorem tryDivide_multiply (a b : Peano) : tryDivide (b * a) b = some a :=
...
theorem multiply_divide_associative (x y z : Peano) (h : Divisible y z) :
  ...
  let h2 : Divisible (x * y) z := multiply_divide_associative_h h
  refine ⟨h2, ?_⟩
  exact multiply_cancel_left z (x * divide y z h) (divide (x * y) z h2) (by
    rw [←multiply_associative]
    have hzx : z * x = x * z := multiply_commutative z x
    rw [hzx, multiply_associative, divide_correct y z h, divide_correct (x * y) z h2])
...
theorem divide_lt_of_lt {x y z : Peano}
  ...
  have hmul' : divide y z h2 * z < divide x z h1 * z := by
    rw [multiply_commutative (divide y z h2) z]
    rw [multiply_commutative (divide x z h1) z]
    exact hmul
...
example : compare (1 : Peano) (2 : Peano) = .lt := rfl
