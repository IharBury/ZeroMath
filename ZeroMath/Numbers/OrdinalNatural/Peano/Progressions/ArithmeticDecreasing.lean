...
theorem subtractiveCommonDifference_eq_of_equivalence_of_length_ge_two
  ...
  have hleFromP :
    ...
    have hEq := Sequences.Progression.getLengthFrom_eq_of_current_eq (toProgression p).next hfP hAccP
    rwa [hEq] at hle'
  have hleFromQ :
      CardinalNatural.Peano.fromOrdinal Peano.one.successor ≤
...
def nextMaskedWalkElement (subtractiveCommonDifference limit : Peano) :
...
def agreesWithMaskedElementsFromCurrent
    (subtractiveCommonDifference limit : Peano) (current : Option Peano) :
...
theorem nextMaskedWalkElement_eq_toProgression_next
    (p : ArithmeticDecreasing) (x : Peano) :
...
theorem agreesWithMaskedElementsFromCurrent_eq_true_iff
    (p : ArithmeticDecreasing) (index : Peano) (current : Option Peano)
    (elements : Sequences.List (Option Peano))
    (hcur : current =
      Sequences.Progression.tryGetElement index (toProgression p)) :
    agreesWithMaskedElementsFromCurrent p.subtractiveCommonDifference p.limit
...
theorem agreesWithMaskedElementsFrom_unmasked_eq_true
    (p : ArithmeticDecreasing) (index x : Peano)
...
theorem getLength_of_trySubtract_multiply (first diff n last : Peano)
    ...
    getLength {
      ...
      CardinalNatural.Peano.fromOrdinal n.successor := by
  have hadd : first = n * diff + last :=
    eq_of_trySubtract_add (n * diff) first last h
  have hlt : last < first := by
    rw [hadd]
    exact lt_add_right (n * diff) last
  have hget :=
    getLength_eq_lengthFromGap_of_gt first diff last hlt
  have hsub : subtract first last hlt = n * diff := by
...
def tryExtendToLength (p : ArithmeticDecreasing)
  ...
  | some first =>
    ...
    | .zero =>
      False.elim
        (CardinalNatural.Peano.not_two_le_zero
          (CardinalNatural.Peano.eq_zero_of_le_zero (getLength p) hle ▸ hge))
    | .successor .zero =>
      False.elim
        (CardinalNatural.Peano.not_two_le_one
          (CardinalNatural.Peano.le_trans hge hle))
...
theorem getLength_of_tryExtendToLength (p : ArithmeticDecreasing)
  ...
  · next first hf =>
    ...
    · next hle0 =>
      exact (CardinalNatural.Peano.not_two_le_zero
        (CardinalNatural.Peano.eq_zero_of_le_zero (getLength p) hle0 ▸
          hge)).elim
    · next hle1 =>
      exact (CardinalNatural.Peano.not_two_le_one
        (CardinalNatural.Peano.le_trans hge hle1)).elim
    · next m hle2 =>
      split at h
      · next => nomatch h
      · next last hs =>
        ...
        change
            CardinalNatural.Peano.fromOrdinal
                (CardinalNatural.Peano.toOrdinal m.successor
                  (CardinalNatural.Peano.successor_ne_zero m)).successor =
              CardinalNatural.Peano.successor
                (CardinalNatural.Peano.successor m)
        simp only [CardinalNatural.Peano.fromOrdinal,
        ...
        have hadd :
            first' =
              CardinalNatural.Peano.toOrdinal m.successor
                ...
                p.subtractiveCommonDifference +
                last :=
          eq_of_trySubtract_add (CardinalNatural.Peano.toOrdinal m.successor
              (CardinalNatural.Peano.successor_ne_zero m) *
              p.subtractiveCommonDifference) first' last hs
        have hle_lim : last ≤ first' := by
          rw [hadd]
          exact Or.inl (lt_add_right _ last)
    ...
    · next first hf =>
      exact getLength_lastElementFrom first p.subtractiveCommonDifference
        ...
        (getElementsFrom_length_of_le_getLength p first hf
          (CardinalNatural.Peano.successor n) hleLen)
