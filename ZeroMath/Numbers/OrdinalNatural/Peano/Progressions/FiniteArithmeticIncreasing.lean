  ...
  | successor n ih =>
    ...
    | some y =>
      ...
      · simp only [hle, ↓reduceIte] at hnext
        injection hnext with heq
        have hn : n ≤ y := ih y hm
        have hsucc : successor n ≤ successor y := successor_le_successor hn
...
theorem toProgression_finite (p : FiniteArithmeticIncreasing) :
  ...
  | some x =>
    ...
    exact (not_successor_le p.limit (le_trans hle_idx hle_lim)).elim
      ...
      case some =>
        ...
        | .less hlt =>
          ...
          | .less hdiff =>
            ...
            have hlen :=
              lengthFromGap_successor_of_lt p.commonDifference
...
theorem getElementFrom_successor (first commonDifference : Peano) (n : Peano) :
...
theorem next_eq_some_of_successor_le_getLengthFrom (p : FiniteArithmeticIncreasing)
    (x : Peano) (n : Peano)
  ...
  | none =>
    ...
    have hzero :
        Sequences.Progression.getLengthFrom (toProgression p).next
          ...
          CardinalNatural.Peano.zero := by
      have hEq := Sequences.Progression.getLengthFrom_eq_of_current_eq (toProgression p).next hnext hAcc'
      rw [hEq, Sequences.Progression.getLengthFrom_none]
...
theorem getElement_eq (p : FiniteArithmeticIncreasing) (index : Peano)
  ...
  · next first hf =>
    ...
    have hcur :=
      Sequences.Progression.getElementFrom_eq_of_current_eq (toProgression p).next hf hAcc index
        (by
          dsimp only [Sequences.Progression.getLength] at hle_prog
          exact hle_prog)
    exact (Sequences.Progression.getElementFrom_eq_of_acc_eq (toProgression p).next (some first)
...
a `Progression`. -/
def effectiveFirst (p : FiniteArithmeticIncreasing) : Option Peano :=
  match p.first with
  | none => none
  | some x => if x ≤ p.limit then some x else none
...
theorem getLength_eq_zero_iff_effectiveFirst_none (p : FiniteArithmeticIncreasing) :
  ...
  · intro hlen
    ...
    | some first =>
      ...
      | .greater hgt =>
        simp only [effectiveFirst, hf]
        have : ¬ first ≤ p.limit := not_le_of_gt hgt
      ...
      | .less hlt =>
        simp only [hc] at hlen
        exact (lengthFromGap_ne_zero p.commonDifference _ hlen).elim
  · intro hfirst
    match hf : p.first with
    | none =>
...
theorem not_getLength_zero_of_effectiveFirst_some (p : FiniteArithmeticIncreasing)
...
theorem effectiveFirst_eq_some_of_positive_length (p : FiniteArithmeticIncreasing)
...
theorem effectiveFirst_eq_of_equivalence (p q : FiniteArithmeticIncreasing)
...
theorem getLength_eq_of_equivalence (p q : FiniteArithmeticIncreasing)
...
theorem commonDifference_eq_of_equivalence_of_length_ge_two
    (p q : FiniteArithmeticIncreasing) (first : Peano) (n : CardinalNatural.Peano)
    (hp : effectiveFirst p = some first) (hq : effectiveFirst q = some first)
    (hlenP : getLength p =
...
theorem lastElementFrom_successor_successor (first commonDifference : Peano)
...
theorem first_lt_lastElementFrom_of_ge_two (first commonDifference : Peano)
  ...
  | successor n ih =>
    rw [lastElementFrom_successor_successor]
    exact lt_trans (lt_add_left first commonDifference)
      (ih (first + commonDifference))

theorem first_le_lastElementFrom_of_positive (first commonDifference : Peano)
...
theorem getElementsFrom_successor_successor (first commonDifference : Peano)
    (n : CardinalNatural.Peano) :
      ...
      | successor k =>
        ...
        have hlast_eq :
            lastElementFrom first commonDifference
              ...
              lastElementFrom (first + commonDifference) commonDifference
                (CardinalNatural.Peano.successor k.successor) :=
          lastElementFrom_successor_successor first commonDifference k.successor
        have hlen' :
            getLength {
        ...
        have hdiff_lt :=
          commonDifference_lt_gap_of_add_lt first commonDifference
            ...
            hlt hlt_add
        have hgap_succ :=
          lengthFromGap_successor_of_lt commonDifference
            (subtract
              (lastElementFrom first commonDifference
                (CardinalNatural.Peano.successor
                  (CardinalNatural.Peano.successor k.successor)))
...
theorem effectiveFirst_lastElementFrom (first commonDifference : Peano)
  ...
  have hle := first_le_lastElementFrom_of_positive first commonDifference n hne
...
theorem tryFromElements_getElements (p : FiniteArithmeticIncreasing)
  ...
  obtain ⟨m, hlen⟩ := CardinalNatural.Peano.eq_successor_successor_of_two_le (getLength p) hge
  ...
  | .firstElement x (.firstElement y ys) =>
    ...
    | some diff =>
      ...
      | some last =>
        ...
        have hle : x ≤ last := by
          have :=
            first_le_lastElementFrom_of_positive x diff
...
theorem getElementFrom_eq_add_multiply (first commonDifference : Peano) (n : Peano) :
    getElementFrom first commonDifference n.successor =
  ...
  | successor n ih =>
    calc
      ...
      _ = first + (n * commonDifference + commonDifference) := by rw [add_associative]
      _ = first + (successor n) * commonDifference := by rw [successor_multiply]
...
theorem getElementFrom_of_tryFirstFromIndexedElement
  ...
  | .successor n =>
    ...
    rw [getElementFrom_eq_add_multiply, add_commutative, helement]

/-- Advancing from `index` to a larger `index'` adds
`(index' - index) * commonDifference` to the element. -/
theorem getElementFrom_add_multiply_of_lt (first commonDifference index index' : Peano)
    (hlt : index < index') :
  ...
  | .one, .successor n =>
    ...
    rw [getElementFrom_one, getElementFrom_eq_add_multiply, hsub]
  | .successor m, .one =>
  ...
  | .successor m, .successor n =>
    ...
    rw [getElementFrom_eq_add_multiply, getElementFrom_eq_add_multiply, hsub]
    ...
    calc
      ...
      _ = first +
            ...
            rw [multiply_commutative (m + subtract n m hlt'), multiply_add,
              multiply_commutative commonDifference m,
              multiply_commutative commonDifference (subtract n m hlt')]
      _ = first + m * commonDifference +
            (subtract n m hlt') * commonDifference := by rw [← add_associative]
...
theorem getElementFrom_of_tryFirst_tryCommonDifference
  ...
  rw [getElementFrom_add_multiply_of_lt first diff index index' hlt, h1, hgap]
...
theorem tryCommonDifferenceFromOrderedIndexedElements_getElementFrom
    (first commonDifference index index' : Peano) (hlt : index < index') :
  ...
  have heq :=
    getElementFrom_add_multiply_of_lt first commonDifference index index' hlt
  have hsub :
      trySubtract (getElementFrom first commonDifference index')
        (getElementFrom first commonDifference index) =
      some ((subtract index' index hlt) * commonDifference) := by
    rw [heq]
    exact trySubtract_self_add _ _
  simp only [hsub]
  exact tryDivide_multiply commonDifference (subtract index' index hlt)
...
theorem tryFirstFromIndexedElement_getElementFrom
    (first commonDifference index : Peano) :
  ...
  | .successor n =>
    simp only [tryFirstFromIndexedElement, getElementFrom_eq_add_multiply]
    exact trySubtract_add_right first (n * commonDifference)
...
theorem tryFromTwoElementsAndLength_getElement
  ...
  · simp only [tryFromTwoElementsAndLength, hle1, hle2, ↓reduceIte, hget1, hget2]
    ...
    | .greater hgt =>
      ...
      rfl
  · have hfq :=
      effectiveFirst_lastElementFrom first p.commonDifference (getLength p) hne0
    have hlenq :=
...
def agreesWithMaskedElementsFrom (p : FiniteArithmeticIncreasing)
    (index : Peano) (elements : Sequences.List (Option Peano)) : Bool :=
  match effectiveFirst p with
  | none =>
...
length equals that of the list.
...
theorem nextMaskedWalkElement_eq_toProgression_next
    (p : FiniteArithmeticIncreasing) (x : Peano) :
...
theorem agreesWithMaskedElementsFromCurrent_eq_true_iff
    (p : FiniteArithmeticIncreasing) (index : Peano) (current : Option Peano)
    (elements : Sequences.List (Option Peano))
    (hcur : current =
      Sequences.Progression.tryGetElement index (toProgression p)) :
...
theorem agreesWithMaskedElementsFrom_unmasked_eq_true
    (p : FiniteArithmeticIncreasing) (index x : Peano)
...
theorem getLength_agreesWithMaskedElementsFrom_of_tryFromMaskedElementsGivenOne
    (index1 element1 : Peano) (length : CardinalNatural.Peano)
    (index : Peano) (hlt : index1 < index)
    (elements : Sequences.List (Option Peano))
      ...
      · next hAgree =>
        ...
        have hsound :=
          getLength_getElement_of_tryFromTwoElementsAndLength
            index1 element1 index element2 length (ne_of_lt hlt) p hs
        have htry1 :
            Sequences.Progression.tryGetElement index1 (toProgression p) =
              some element1 := by
...
theorem getLength_agreesWithMaskedElementsFrom_of_tryFromMaskedElementsFrom
    (index : Peano) (length : CardinalNatural.Peano)
    (elements : Sequences.List (Option Peano))
    (hge : CardinalNatural.Peano.two ≤ elements.unmaskedCount)
    (p : FiniteArithmeticIncreasing)
...
theorem getLength_agreesWithMaskedElements_of_tryFromMaskedElements
  ...
  have h' :
      tryFromMaskedElementsFrom one elements.length elements hge = some p := by
    simpa only [tryFromMaskedElements] using h
  have hsound :=
    getLength_agreesWithMaskedElementsFrom_of_tryFromMaskedElementsFrom
      one elements.length elements hge p h'
  refine ⟨hsound.1, ?_⟩
...
theorem InfiniteArithmetic_getElement_eq_getElementFrom
    (first commonDifference : Peano) (index : Peano) :
...
theorem effectiveFirst_extendToLength (p : FiniteArithmeticIncreasing)
  ...
  · next first' hf' =>
    ...
    injection heq with heq'
    rw [← heq']
    exact effectiveFirst_lastElementFrom first p.commonDifference length hne
...
theorem effectiveFirst_truncateToLength (p : FiniteArithmeticIncreasing)
  ...
  | successor n =>
    ...
    · next first' hf' =>
      ...
      injection heq with heq'
      rw [← heq']
      exact effectiveFirst_lastElementFrom first p.commonDifference
        (CardinalNatural.Peano.successor n)
        (fun h => nomatch h)
...
theorem getElement_truncateToLength (p : FiniteArithmeticIncreasing)
    ...
    ∃ (hle' : CardinalNatural.Peano.fromOrdinal index ≤
      ...
      getElement (truncateToLength p hge length hleLen) index hle' =
        getElement p index
          (CardinalNatural.Peano.le_trans hle hleLen) := by
  have hleOrig :
      CardinalNatural.Peano.fromOrdinal index ≤ getLength p :=
    CardinalNatural.Peano.le_trans hle hleLen
  have hle' :
      CardinalNatural.Peano.fromOrdinal index ≤
        getLength (truncateToLength p hge length hleLen) :=
...
theorem getLength_truncateInfiniteToLength (p : InfiniteArithmetic)
    (length : CardinalNatural.Peano) :
    getLength (truncateInfiniteToLength p length) = length := by
  cases length with
  | zero =>
    rfl
  | successor n =>
...
theorem effectiveFirst_truncateInfiniteToLength (p : InfiniteArithmetic)
    (length : CardinalNatural.Peano)
    (hne : length ≠ CardinalNatural.Peano.zero) :
    effectiveFirst (truncateInfiniteToLength p length) = some p.first := by
  cases length with
  | zero =>
    exact (hne rfl).elim
  | successor n =>
...
theorem getElement_truncateInfiniteToLength (p : InfiniteArithmetic)
    ...
    (hle : CardinalNatural.Peano.fromOrdinal index ≤ length) :
    ∃ (hle' : CardinalNatural.Peano.fromOrdinal index ≤
        getLength (truncateInfiniteToLength p length)),
      getElement (truncateInfiniteToLength p length) index hle' =
        InfiniteArithmetic.getElement p index := by
  have hle' :
      CardinalNatural.Peano.fromOrdinal index ≤
        getLength (truncateInfiniteToLength p length) :=
    (getLength_truncateInfiniteToLength p length).symm ▸ hle
  refine ⟨hle', ?_⟩
  have hne : length ≠ CardinalNatural.Peano.zero := by
    intro hzero
    exact CardinalNatural.Peano.fromOrdinal_ne_zero index
      (CardinalNatural.Peano.eq_zero_of_le_zero _ (hzero ▸ hle))
  have hf := effectiveFirst_truncateInfiniteToLength p length hne
  rw [getElement_eq_getElementFrom (truncateInfiniteToLength p length)
    p.first hf index hle']
  have hdiff :
      (truncateInfiniteToLength p length).commonDifference =
        p.commonDifference := by
    cases length with
    | zero =>
      exact (hne rfl).elim
